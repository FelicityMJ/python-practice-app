import { REVISION_CARDS, FLUENCY_SPRINTS } from "./revision-fluency-v8-1-2.js";
...
// v8.0: required six-attempt fluency sprints across SDD, DDD and CS.
ACTIVITIES.push(...FLUENCY_SPRINTS);
...
const MASTERY_VERSION = "8.3";
const COURSE_ID = "nat5-computing";
const LEARNING_ROUTES = Object.freeze({ FULL: "full-course", ACCELERATOR: "exam-accelerator" });
console.info(`Nat 5 Computing v${APP_VERSION}: Mastery Edition + Computing Arcade + school email/password sign-in`);
...
let learningRoute = LEARNING_ROUTES.FULL;
let currentPreferences = {};
let dailyMasterySession = null;
...
let revisionQuestionStartedAt = 0;
...
let quickArcadeState = null;
let arcadePracticeMode = "unlocked";
...
function showView(viewId) {
  ["authView", "teacherView", "pupilView", "dailyMasteryView", "masteryDiagnosticView", "gamesView", "debugGameView", "logicGameView", "quickArcadeView", "spacedView", "revisionView", "peerReviewView", "activityView", "challengeView"].forEach(id => {
...
function isIndependentLearner() {
  return currentProfile?.role === "student" && currentProfile?.accountType === "individual";
}

function hasSchoolClass() {
  return currentProfile?.role === "student" && typeof currentProfile?.classId === "string" && currentProfile.classId.length > 0;
}

function learningScopeId() {
  if (hasSchoolClass()) return currentProfile.classId;
  if (auth.currentUser?.uid) return `individual_${auth.currentUser.uid}`;
  return "individual";
}

function entitlementIsActive(entitlement) {
  if (!entitlement || entitlement.status !== "active") return false;
  if (entitlement.permanent === true) return true;
  if (!entitlement.endsAt) return false;
  const end = typeof entitlement.endsAt.toDate === "function" ? entitlement.endsAt.toDate() : new Date(entitlement.endsAt);
...
async function loadCurrentEntitlement() {
  ...
  try {
    const snapshot = await getDoc(doc(db, "users", auth.currentUser.uid, "entitlements", COURSE_ID));
    if (snapshot.exists()) currentEntitlement = snapshot.data();
  } catch (error) {
    console.warn("Course entitlement could not be loaded:", error);
  }
  return currentEntitlement;
}

function licenceLabel() {
  if (currentEntitlement?.source === "founding-complimentary") return "Founding complimentary access";
  if (currentEntitlement?.source === "individual") return "Individual study licence";
  if (currentEntitlement?.source === "school") return "School licence";
  if (hasSchoolClass()) return "School pilot access";
...
const MASTERY_AREA_META = Object.freeze({
  sdd: { name: "Python", prefix: "P", icon: "🐍" },
  ddd: { name: "SQL", prefix: "Q", icon: "🗃️" },
  cs: { name: "Computer Systems", prefix: "C", icon: "💻" }
});

const INSTRUCTIONAL_ACTIVITY_TYPES = new Set(["lesson", "video", "visualiser"]);
// These activities are genuinely useful assessment practice, but the browser cannot
// objectively decide whether a pupil's extended written response deserves full marks.
// A completed self-review therefore counts as "secure enough to advance" without
// pretending it is retained machine-verified mastery.
const SELF_REVIEW_GATE_TYPES = new Set(["exam-style", "official-paper"]);

function activitySkillIds(activity) {
  return Array.isArray(activity?.skills) ? activity.skills.filter(Boolean) : [];
}

function legacyCompletionCountsAsMastery(progress) {
  return Boolean(progress?.completed && progress?.mastered === undefined && !progress?.masteryVersion);
}

function reviewStateRetained(skillId) {
  const state = currentReviewStates.get(skillId);
  return Boolean(state && Number(state.stage || 0) >= 3 && Number(state.consecutiveCorrect || 0) >= 2 && state.lastCorrect !== false);
}

function activityHasRetainedSkills(activity) {
  const skills = activitySkillIds(activity);
  return Boolean(skills.length && skills.every(reviewStateRetained));
}

function isActivityMastered(activity, progress = currentProgress.get(activity?.id)) {
  if (!activity || !progress) return false;
  if (progress.mastered === true) return true;
  if (legacyCompletionCountsAsMastery(progress)) return true;
  if (INSTRUCTIONAL_ACTIVITY_TYPES.has(activity.type) && progress.completed) return true;
  if (SELF_REVIEW_GATE_TYPES.has(activity.type) && progress.completed) return true;
  if (progress.completed && activityHasRetainedSkills(activity)) return true;
  return false;
}

function activityMasteryLabel(activity, progress = currentProgress.get(activity?.id)) {
  if (!progress) return { key: "not-started", label: "Not studied", icon: "○" };
  if (activityHasRetainedSkills(activity) && isActivityMastered(activity, progress)) return { key: "retained", label: "Retained", icon: "◆" };
  if (isActivityMastered(activity, progress)) return { key: "mastered", label: "Mastered", icon: "✓" };
  if (progress.masteryStatus === "repair" || Number(progress.incorrectStreak || 0) >= 2) return { key: "repair", label: "Needs repair", icon: "!" };
  if (progress.completed) return { key: "developing", label: "Completed · developing", icon: "~" };
  if (Number(progress.attempts || 0) > 0 || progress.status === "in_progress") return { key: "learning", label: "Learning", icon: "◐" };
  return { key: "ready", label: "Ready", icon: "○" };
}

function masteryGateSatisfied(activity) {
  if (!activity) return true;
  const progress = currentProgress.get(activity.id);
  if (isActivityMastered(activity, progress)) return true;
  // Lessons/visual explanations and self-reviewed exam work are progression gates.
  // Machine-marked practice still has to meet the stronger mastery rules above.
  return Boolean((INSTRUCTIONAL_ACTIVITY_TYPES.has(activity.type) || SELF_REVIEW_GATE_TYPES.has(activity.type)) && progress?.completed);
}

function placementBypassesActivity(activity) {
  if (!activity) return false;
  const placed = currentPreferences?.masteryPlacement?.[activity.areaId];
  if (!placed?.unitId) return false;
  const placedUnit = unitById(placed.unitId);
  const activityUnit = unitById(activity.unitId);
  return Boolean(placedUnit && activityUnit && Number(activityUnit.order || 0) < Number(placedUnit.order || 0));
}

function prerequisiteSatisfied(activity) {
  return masteryGateSatisfied(activity) || placementBypassesActivity(activity);
}

function computeMasteryAttemptFields(activity, old = {}, { countAttempt = false, correct = false, completed = false, explicitMastered } = {}) {
  const attempts = countAttempt ? Number(old.attempts || 0) + 1 : Number(old.attempts || 0);
  const consecutiveCorrect = countAttempt ? (correct ? Number(old.consecutiveCorrect || 0) + 1 : 0) : Number(old.consecutiveCorrect || 0);
  const incorrectStreak = countAttempt ? (correct ? 0 : Number(old.incorrectStreak || 0) + 1) : Number(old.incorrectStreak || 0);
  let mastered = old.mastered === true || legacyCompletionCountsAsMastery(old);

  if (explicitMastered !== undefined) mastered = Boolean(explicitMastered);
  else if (countAttempt && correct) {
    const noHistoricHints = Number(old.hintsUsed || 0) === 0;
    const cleanFirstPass = attempts === 1 && noHistoricHints;
    const repeatedSuccess = consecutiveCorrect >= 2;
    if (cleanFirstPass || repeatedSuccess) mastered = true;
  }

  if (countAttempt && !correct && incorrectStreak >= 2 && (old.mastered === true || legacyCompletionCountsAsMastery(old))) mastered = false;
  if ((INSTRUCTIONAL_ACTIVITY_TYPES.has(activity?.type) || SELF_REVIEW_GATE_TYPES.has(activity?.type)) && completed) mastered = true;

  let masteryStatus = old.masteryStatus || "learning";
  if (mastered) masteryStatus = activityHasRetainedSkills(activity) ? "retained" : "mastered";
  else if (incorrectStreak >= 2) masteryStatus = "repair";
  else if (completed || attempts > 0) masteryStatus = "developing";
  ...
  return {
    ...
    masteredAt: mastered && old.mastered !== true ? new Date().toISOString() : (old.masteredAt || null)
  };
}

function isActivityUnlocked(activity) {
  if (learningRoute === LEARNING_ROUTES.ACCELERATOR) return true;
  const progress = currentProgress.get(activity.id);
  if (isActivityMastered(activity, progress) || progress?.completed) return true;
  if (activity.required !== false) {
    const previous = previousRequiredActivity(activity);
    return !previous || prerequisiteSatisfied(previous);
  }
  const prerequisites = activity.prerequisiteIds || [];
  if (prerequisites.length) {
    return prerequisites.every(id => {
      const prerequisite = allActivities.find(item => item.id === id);
      return prerequisite ? prerequisiteSatisfied(prerequisite) : currentProgress.get(id)?.mastered === true;
    });
  }
...
async function activeTeacherInvite(email) {
  const normalised = normaliseEmail(email);
  if (!normalised) return null;
  const invite = await getDoc(doc(db, "teacherInvites", normalised));
  if (!invite.exists() || invite.data().active !== true) return null;
  return invite.data();
}
...
async function ensureTeacherProfile(user) {
  ...
  if (!email) throw new Error("Your teacher account does not have an email address.");
  if (!user.emailVerified) {
    throw new Error("Verify your school email address before opening the teacher dashboard.");
  }

  const invite = await activeTeacherInvite(email);
  if (!invite) {
    throw new Error("This email address has not been approved as a teacher yet.");
  }
...
async function loadProfile(user) {
  ...
  if (!snapshot.exists() && user.email) {
    const invite = await activeTeacherInvite(user.email);
    if (invite) {
...
async function routeSignedInUser(user) {
  try {
    ...
    const usesPasswordSignIn = providerIds.has("password");
    const isSchoolPupil = currentProfile.role === "student" && currentProfile.accountType !== "individual";
    const isTeacherAccount = currentProfile.role === "teacher";
    if ((isSchoolPupil || isTeacherAccount) && usesPasswordSignIn && !user.emailVerified) {
      try { await sendEmailVerification(user); } catch (verificationError) { console.warn("Verification email could not be resent:", verificationError); }
      showMessage("Please verify your school email address first. We have sent another verification email; open the link in that email, then sign in again.", "info");
      await signOut(auth);
      return;
    }

    await loadCurrentEntitlement();
    if (currentProfile.role === "student" && currentProfile.accountType === "individual" && !entitlementIsActive(currentEntitlement)) {
      throw new Error("This individual account does not currently have an active Nat 5 Computing licence.");
    }
...
onAuthStateChanged(auth, async user => {
  if (user) {
    ...
    currentProfile = null;
    currentEntitlement = null;
    learningRoute = LEARNING_ROUTES.FULL;
    currentTask = null;
...
function friendlyAuthMessage(error, fallback) {
  ...
  if (code.includes("email-already-in-use")) return "An account already exists for that email address. Sign in instead, or use Forgot password?";
  if (code.includes("invalid-email")) return "Enter a valid email address.";
  if (code.includes("weak-password")) return "Choose a stronger password containing at least 8 characters.";
  if (code.includes("too-many-requests")) return "Firebase has temporarily limited password or sign-in requests. Wait a few minutes, then try again.";
  if (code.includes("operation-not-allowed")) return "Email/password sign-in is not enabled in Firebase Authentication.";
  if (code.includes("network-request-failed")) return "The request could not reach Firebase. Check the internet connection and try again.";
  return error?.message || fallback;
}

async function activeJoinCodeData(code) {
  const snapshot = await getDoc(doc(db, "joinCodes", code));
  if (!snapshot.exists() || snapshot.data().active !== true) throw new Error("That class code is not valid.");
  return snapshot.data();
}

async function createSchoolPupilProfile(user, joinData, code, displayName) {
  const email = normaliseEmail(user.email);
  if (!email) throw new Error("Your sign-in account did not provide an email address.");
  ...
  batch.set(doc(db, "classes", joinData.classId, "members", user.uid), {
    userId: user.uid,
    role: "student",
    username: email,
    email,
...
function setTeacherAuthTab(tab) {
  const login = tab === "login";
  elements.teacherLoginTab.classList.toggle("active", login);
  elements.teacherRegisterTab.classList.toggle("active", !login);
  elements.teacherLoginForm.classList.toggle("hidden", !login);
  elements.teacherRegisterForm.classList.toggle("hidden", login);
}

elements.teacherLoginTab.addEventListener("click", () => setTeacherAuthTab("login"));
elements.teacherRegisterTab.addEventListener("click", () => setTeacherAuthTab("register"));

elements.teacherLoginForm.addEventListener("submit", async event => {
  event.preventDefault();
  clearMessage();
  const email = normaliseEmail(elements.teacherLoginEmail.value);
  const password = elements.teacherLoginPassword.value;
  if (!email || !password) {
    showMessage("Enter your approved school email address and password.", "error");
    return;
  }
  ...
  try {
    ...
    if (!user.emailVerified) {
      try { await sendEmailVerification(user); } catch (verificationError) { console.warn("Teacher verification email could not be resent:", verificationError); }
      await signOut(auth);
      showMessage("Please verify your school email address first. We have sent another verification email.", "info");
      return;
    }

    const profileSnapshot = await getDoc(doc(db, "users", user.uid));
    if (profileSnapshot.exists() && profileSnapshot.data().role !== "teacher") {
      await signOut(auth);
      showMessage("That account is not registered as a teacher. Use the appropriate pupil sign-in instead.", "error");
      return;
    }
    if (!profileSnapshot.exists()) await ensureTeacherProfile(user);
    accountSetupInProgress = false;
    await routeSignedInUser(user);
  } catch (error) {
    console.error(error);
    try { if (auth.currentUser) await signOut(auth); } catch (_) { /* ignore cleanup */ }
    showMessage(friendlyAuthMessage(error, "The teacher account could not be opened."), "error");
  } finally {
    accountSetupInProgress = false;
  }
});

elements.teacherRegisterForm.addEventListener("submit", async event => {
  event.preventDefault();
  clearMessage();
  const email = normaliseEmail(elements.teacherRegisterEmail.value);
  const password = elements.teacherRegisterPassword.value;
  const confirmation = elements.teacherRegisterPasswordConfirm.value;

  if (!email) {
    showMessage("Enter the school email address that has been approved for teacher access.", "error");
    return;
  }
  ...
  try {
    ...
    if (!invite) {
      ...
      newUser = null;
      throw new Error("This email address has not been approved for teacher access yet.");
    }

    await sendEmailVerification(newUser);
    await signOut(auth);
    setTeacherAuthTab("login");
    elements.teacherLoginEmail.value = email;
    showMessage("Teacher account created. Check your school email and click the verification link, then come back and sign in.", "success");
  } catch (error) {
    console.error(error);
    if (newUser && auth.currentUser?.uid === newUser.uid) {
      try { await signOut(auth); } catch (_) { /* ignore cleanup */ }
    }
    showMessage(friendlyAuthMessage(error, "The teacher account could not be created."), "error");
  } finally {
    accountSetupInProgress = false;
  }
});

elements.teacherForgotPasswordButton.addEventListener("click", async () => {
  clearMessage();
  const email = normaliseEmail(elements.teacherLoginEmail.value);
  if (!email) {
    showMessage("Enter your teacher email address first, then choose Forgot password?", "info");
    elements.teacherLoginEmail.focus();
    return;
  }
  elements.teacherForgotPasswordButton.disabled = true;
  try {
    await sendPasswordResetEmail(auth, email);
    showMessage("If a teacher account exists for that email address, a password-reset email has been sent.", "success");
  } catch (error) {
    console.error(error);
    showMessage(friendlyAuthMessage(error, "The password-reset email could not be sent."), "error");
  } finally {
    elements.teacherForgotPasswordButton.disabled = false;
  }
});
...
elements.pupilLoginForm.addEventListener("submit", async event => {
  event.preventDefault();
  clearMessage();
  const email = normaliseEmail(elements.pupilLoginEmail.value);
  const password = elements.pupilLoginPassword.value;
  if (!email || !password) {
    showMessage("Enter your school email address and password.", "error");
    return;
  }
  ...
  try {
    ...
    if (!profileSnapshot.exists()) {
      await signOut(auth);
      showMessage("That account has not joined a class yet. Choose Join a class and use your class code first.", "info");
      return;
    }
    const profile = profileSnapshot.data();
    if (profile.role !== "student" || profile.accountType === "individual") {
      await signOut(auth);
      showMessage(profile.role === "teacher" ? "That account is registered as a teacher. Use Teacher sign in." : "Use Personal Study for that account.", "error");
      return;
    }
    if (!user.emailVerified) {
      try { await sendEmailVerification(user); } catch (verificationError) { console.warn("Pupil verification email could not be resent:", verificationError); }
      await signOut(auth);
      showMessage("Please verify your school email address first. We have sent another verification email.", "info");
      return;
    }
    ...
    console.error(error);
    try { if (auth.currentUser) await signOut(auth); } catch (_) { /* ignore cleanup */ }
    showMessage(friendlyAuthMessage(error, "The classroom pupil account could not be opened."), "error");
  } finally {
    accountSetupInProgress = false;
  }
});

elements.pupilForgotPasswordButton.addEventListener("click", async () => {
  clearMessage();
  const email = normaliseEmail(elements.pupilLoginEmail.value);
  if (!email) {
    showMessage("Enter your school email address first, then choose Forgot password?", "info");
    elements.pupilLoginEmail.focus();
    return;
  }
  elements.pupilForgotPasswordButton.disabled = true;
  try {
    await sendPasswordResetEmail(auth, email);
    showMessage("If an account exists for that email address, a password-reset email has been sent. Check your inbox and junk folder.", "success");
  } catch (error) {
    console.error(error);
    showMessage(friendlyAuthMessage(error, "The password-reset email could not be sent."), "error");
  } finally {
    elements.pupilForgotPasswordButton.disabled = false;
  }
});
...
elements.pupilRegisterForm.addEventListener("submit", async event => {
  ...
  const email = normaliseEmail(elements.registerEmail.value);
  const password = elements.registerPassword.value;
  const confirmation = elements.registerPasswordConfirm.value;

  if (code.length < 5 || displayName.length < 1) {
    showMessage("Check the class code and display name.", "error");
    return;
  }
  if (!email) {
    showMessage("Enter your school email address.", "error");
    return;
  }
  if (password.length < 8) {
    showMessage("Choose a password containing at least 8 characters.", "error");
  ...
  accountSetupInProgress = true;
  try {
    const joinData = await activeJoinCodeData(code);
    const credential = await createUserWithEmailAndPassword(auth, email, password);
    newUser = credential.user;
    await createSchoolPupilProfile(newUser, joinData, code, displayName);
    profileCreated = true;
    ...
    try {
      ...
      console.warn("Initial verification email could not be sent:", verificationError);
    }

    await signOut(auth);
    accountSetupInProgress = false;
    elements.pupilLoginTab.click();
    elements.pupilLoginEmail.value = email;
    showMessage(
      verificationSent
        ? "Account created. Check your school email and click the verification link, then come back here and sign in."
        : "Account created, but the verification email could not be sent just now. Try signing in again and we will resend it.",
    ...
    showMessage(friendlyAuthMessage(error, "The pupil account could not be created."), "error");
  } finally {
    accountSetupInProgress = false;
  }
});

elements.individualLoginForm.addEventListener("submit", async event => {
  event.preventDefault();
  clearMessage();
  const email = normaliseEmail(elements.loginEmail.value);
  const password = elements.loginPassword.value;

  if (!email || !password) {
    showMessage("Enter the email address and password for your Personal Study licence.", "error");
    return;
  }
  ...
  try {
    ...
    if (!profileSnapshot.exists() || profileSnapshot.data().accountType !== "individual") {
      await signOut(auth);
      showMessage("That is not a Personal Study account. Use Teacher or Classroom pupil sign in instead.", "info");
      return;
    }
    accountSetupInProgress = false;
    ...
    console.error(error);
    try { if (auth.currentUser) await signOut(auth); } catch (_) { /* ignore cleanup */ }
    showMessage(friendlyAuthMessage(error, "The Personal Study account could not be opened."), "error");
  } finally {
    accountSetupInProgress = false;
  }
});

elements.individualForgotPasswordButton.addEventListener("click", async () => {
  clearMessage();
  const email = normaliseEmail(elements.loginEmail.value);
  if (!email) {
    showMessage("Enter your Personal Study email address first, then choose Forgot password.", "info");
    elements.loginEmail.focus();
    return;
  }
  elements.individualForgotPasswordButton.disabled = true;
  try {
    await sendPasswordResetEmail(auth, email);
    showMessage("If that email address has a Personal Study account, a password-reset email has been sent. Check your inbox and junk/spam folder.", "success");
  } catch (error) {
    console.error(error);
    showMessage(friendlyAuthMessage(error, "Firebase could not send the password-reset email. Please try again in a few minutes."), "error");
  } finally {
    elements.individualForgotPasswordButton.disabled = false;
  }
});
...
function renderTeacherLicenceStatus() {
  if (!elements.teacherLicenceStatusCard) return;
  if (currentEntitlement?.permanent === true) {
    elements.teacherLicenceStatusTitle.textContent = licenceLabel();
    elements.teacherLicenceStatusText.textContent = "Permanent teacher and class access is attached to this founding account.";
    elements.teacherLicenceStatusBadge.textContent = "Permanent";
  } else {
    elements.teacherLicenceStatusTitle.textContent = "School pilot access";
    elements.teacherLicenceStatusText.textContent = "Your approved teacher account can create classes and use the current pilot features.";
    elements.teacherLicenceStatusBadge.textContent = "Active";
  }
}

async function loadTeacherDashboard() {
  renderTeacherLicenceStatus();
...
function renderSpacedClassOverview(classItem) {
  const attempts = (classItem.reviewAttempts || []).filter(item => !item.revisionCard);
  const states = (classItem.reviewStates || []).filter(item => !String(item.skillId || "").startsWith("card:"));
...
function renderGamesClassOverview(classItem) {
  ...
  const quickAttempts = attempts.filter(item => !["debug-rescue", "logic-showdown"].includes(item.game));
  ...
  elements.gameClassStats.innerHTML = [
    statCard(attempts.length ? `${rate(attempts)}%` : "—", "Overall Arcade accuracy"),
    statCard(debugAttempts.length ? `${rate(debugAttempts)}%` : "—", "Debug success"),
    statCard(quickAttempts.length ? `${rate(quickAttempts)}%` : "—", "Quick-game accuracy"),
    statCard(weakest ? escapeHtml(SKILL_LABELS[weakest[0]] || weakest[0]) : "—", "Arcade skill to revisit")
  ].join("");
  const rows = classItem.members.map(member => {
    ...
    const quickOwn = own.filter(item => !["debug-rescue", "logic-showdown"].includes(item.game));
    const cabinets = new Set(own.map(item => item.game)).size;
    return { member, own, debugOwn, logicOwn, quickOwn, cabinets };
  ...
  elements.gamePupilStatsBody.innerHTML = rows.map(row => `<tr>
    ...
    <td>${row.own.length ? `${rate(row.own)}%` : "—"}</td>
  ...
  </tr>`).join("") || `<tr><td colspan="6">No Computing Arcade responses yet.</td></tr>`;
}
...
function renderTeacherArcadeControl(classItem) {
  if (!elements.classArcadeStatus || !elements.toggleClassArcadeButton) return;
  const open = classItem.arcadeOpen === true;
  elements.classArcadeStatus.textContent = open ? "OPEN" : "CLOSED";
  elements.classArcadeStatus.className = `status-pill ${open ? "complete" : "working"}`;
  elements.classArcadeDescription.textContent = open
    ? "Pupils in this class can use the Computing Arcade for skills they have reached."
    : "Pupils in this class cannot enter the Computing Arcade until you open it.";
  elements.toggleClassArcadeButton.textContent = open ? "Close Arcade" : "Open Arcade";
  elements.toggleClassArcadeButton.className = open ? "secondary" : "success-button";
  elements.classArcadeSaveStatus.textContent = "";
}
...
function arcadeGameLabel(gameId) {
  return ({
    "debug-rescue": "Debug Rescue",
    "logic-showdown": "Logic Showdown",
    "code-flyer": "Code Flyer",
    "trace-race": "Trace Race",
    "binary-bomb": "Binary Bomb",
    "sql-sprint": "SQL Sprint",
    "cyber-escape": "Cyber Escape"
  })[gameId] || "Computing Arcade";
}

function skillEvidenceForPupil(classItem, userId, skillId) {
  const reviewEvidence = (classItem.reviewAttempts || [])
    .filter(item => item.userId === userId && !item.revisionCard && !item.retry && (item.skillIds || []).includes(skillId))
  ...
  const gameEvidence = (classItem.gameAttempts || [])
    ...
    .map(item => ({
      score: item.correct ? 1 : 0,
      kind: arcadeGameLabel(item.game),
      correct: Boolean(item.correct),
...
function allEvidenceSkills(classItem) {
  ...
  (classItem.reviewAttempts || []).filter(item => !item.revisionCard).forEach(item => (item.skillIds || []).forEach(skill => skills.add(skill)));
...
function showSkillInsight(classItem, member, skillId) {
  ...
  const relatedReviews = (classItem.reviewAttempts || [])
    .filter(item => item.userId === member.userId && !item.revisionCard && (item.skillIds || []).includes(skillId))
    .sort((a, b) => reviewAttemptMillis(b) - reviewAttemptMillis(a))
    .slice(0, 8);
  ...
  elements.skillInsightViewer.innerHTML = `
    ...
    <div class="insight-two-column">
      ...
      <section>
        ...
        <h5>Computing Arcade evidence</h5>
        <div class="review-dot-row">${relatedGames.length ? relatedGames.map(item => `<span class="review-dot ${item.correct ? "correct" : "incorrect"}" title="${escapeHtml(arcadeGameLabel(item.game))} · ${humanDate(item.completedAt)}">${item.correct ? "✓" : "×"}</span>`).join("") : `<span class="help-text">No related game evidence yet.</span>`}</div>
      </section>
    </div>`;
...
function classProgressCountsAsMastered(activity, progress) {
  if (!activity || !progress) return false;
  if (progress.mastered === true) return true;
  if (progress.completed && progress.mastered === undefined && !progress.masteryVersion) return true;
  if ((INSTRUCTIONAL_ACTIVITY_TYPES.has(activity.type) || SELF_REVIEW_GATE_TYPES.has(activity.type)) && progress.completed) return true;
  return false;
}

function teacherWorkingLevel(classItem, userId, areaId) {
  const units = areaUnits(areaId);
  const placementRow = (classItem.progress || []).find(row => row.userId === userId && row.taskId === `MASTERY-PLACEMENT-${areaId}`);
  let bestIndex = placementRow?.placementUnitId ? Math.max(0, units.findIndex(unit => unit.id === placementRow.placementUnitId)) : 0;
  units.forEach((unit, index) => {
    const activities = requiredActivities(areaId).filter(activity => activity.unitId === unit.id);
    if (!activities.length) return;
    const progressRows = activities.map(activity => ({ activity, progress: (classItem.progress || []).find(row => row.userId === userId && row.taskId === activity.id) }));
    const secure = progressRows.filter(row => classProgressCountsAsMastered(row.activity, row.progress)).length;
    const started = progressRows.filter(row => row.progress).length;
    if (secure / activities.length >= 0.5 || (secure > 0 && started >= Math.min(2, activities.length))) bestIndex = Math.max(bestIndex, index);
  });
  const unit = units[bestIndex] || units[0];
  const meta = MASTERY_AREA_META[areaId];
  return unit ? `${meta.prefix}${unit.number}` : `${meta.prefix}1`;
}

function teacherRetentionRepairSummary(classItem, member, skills) {
  let retained = 0;
  let repair = 0;
  skills.forEach(skillId => {
    const evidence = skillEvidenceForPupil(classItem, member.userId, skillId);
    const reviews = (classItem.reviewAttempts || []).filter(item => item.userId === member.userId && !item.revisionCard && (item.skillIds || []).includes(skillId) && !item.retry);
    const successfulReviews = reviews.filter(item => item.correct);
    if (evidence.status === "green" && successfulReviews.length >= 2) retained += 1;
    const relatedProgress = (classItem.progress || []).filter(item => {
      const activity = allActivities.find(candidate => candidate.id === item.taskId);
      return item.userId === member.userId && activity && activitySkillIds(activity).includes(skillId);
    });
    const wasSecure = relatedProgress.some(item => item.mastered === true || (item.completed && item.mastered === undefined && !item.masteryVersion));
    const explicitRepair = relatedProgress.some(item => item.masteryStatus === "repair" || Number(item.incorrectStreak || 0) >= 2);
    if (explicitRepair || (wasSecure && evidence.status === "red")) repair += 1;
  });
  return { retained, repair };
}
...
function renderMasteryHeatmap(classItem) {
  ...
  table.className = "mastery-heatmap mastery-map-v83";
  table.innerHTML = `<thead><tr><th class="sticky-pupil-column">Pupil</th><th class="level-column">Python</th><th class="level-column">SQL</th><th class="level-column">CS</th><th class="level-column" title="Skills with repeated successful delayed retrieval">Retained</th><th class="level-column" title="Previously learned skills that currently need repair">Repair</th>${skills.map(skillId => `<th title="${escapeHtml(SKILL_LABELS[skillId] || skillId)}"><span>${escapeHtml(SKILL_LABELS[skillId] || skillId)}</span></th>`).join("")}</tr></thead><tbody></tbody>`;
  ...
  rows.forEach(rowData => {
    ...
    const summary = teacherRetentionRepairSummary(classItem, rowData.member, skills);
    const levelValues = [
      teacherWorkingLevel(classItem, rowData.member.userId, "sdd"),
      teacherWorkingLevel(classItem, rowData.member.userId, "ddd"),
      teacherWorkingLevel(classItem, rowData.member.userId, "cs"),
      `◆ ${summary.retained}`,
      summary.repair ? `! ${summary.repair}` : "—"
    ];
    levelValues.forEach((value, index) => {
      const cell = document.createElement("td");
      cell.className = `mastery-level-cell ${index === 4 && summary.repair ? "has-repair" : ""}`;
...
function pupilMetrics(classItem, member) {
  ...
  const reviews = (classItem.reviewAttempts || []).filter(item => item.userId === member.userId && !item.revisionCard && !item.retry).sort((a, b) => reviewAttemptMillis(b) - reviewAttemptMillis(a));
...
function showPupilInsight(classItem, member) {
  ...
  elements.pupilInsightPanel.innerHTML = `
    <div class="panel-heading">
      ...
      <div class="card-actions profile-heading-actions">
    ...
    <div class="stats-grid compact pupil-profile-stats">
      ...
      ${statCard(metrics.games.length ? `${metrics.gameAccuracy}%` : "—", "Arcade accuracy")}
...
function showClassDetails(classItem) {
  ...
  renderTeacherArcadeControl(classItem);
  ...
  classItem.members
    ...
    .forEach(member => {
      ...
      row.innerHTML = `
        ...
        <td><div class="pupil-row-actions"><button type="button" class="secondary small open-pupil-profile">Open profile</button></div></td>`;
...
elements.toggleClassArcadeButton?.addEventListener("click", async () => {
  if (!activeTeacherClass) return;
  const nextOpen = activeTeacherClass.arcadeOpen !== true;
  elements.toggleClassArcadeButton.disabled = true;
  elements.classArcadeSaveStatus.textContent = nextOpen ? "Opening…" : "Closing…";
  try {
    const data = { arcadeOpen: nextOpen, arcadeUpdatedAt: serverTimestamp() };
    await setDoc(doc(db, "classes", activeTeacherClass.id), data, { merge: true });
    activeTeacherClass.arcadeOpen = nextOpen;
    renderTeacherArcadeControl(activeTeacherClass);
    elements.classArcadeSaveStatus.textContent = nextOpen ? "Arcade opened for this class" : "Arcade closed for this class";
  } catch (error) {
    console.error(error);
    elements.classArcadeSaveStatus.textContent = "Could not change Arcade access";
  } finally {
    elements.toggleClassArcadeButton.disabled = false;
  }
});
...
async function loadUserPreferences() {
  ...
  try {
    ...
    errorViewMode = ["standard", "friendly"].includes(data.errorMode) ? data.errorMode : "standard";
    learningRoute = Object.values(LEARNING_ROUTES).includes(data.learningRoute) ? data.learningRoute : LEARNING_ROUTES.FULL;
  } catch (error) {
...
async function saveLearningRoutePreference() {
  if (!auth.currentUser || currentProfile?.role !== "student") return;
  const data = { userId: auth.currentUser.uid, learningRoute, updatedAt: serverTimestamp() };
  await setDoc(doc(db, "preferences", auth.currentUser.uid), data, { merge: true });
  currentPreferences = { ...currentPreferences, learningRoute };
}

async function saveErrorPreference() {
  if (!auth.currentUser || currentProfile?.role !== "student") return;
  const data = { userId: auth.currentUser.uid, errorMode: errorViewMode, updatedAt: serverTimestamp() };
  await setDoc(doc(db, "preferences", auth.currentUser.uid), data, { merge: true });
  currentPreferences = { ...currentPreferences, errorMode: errorViewMode };
}
...
async function loadPupilProgress() {
  ...
  const progressItems = snapshot.docs
    .map(item => item.data())
    .filter(item => item.classId === learningScopeId());
...
function activityStatus(activity) {
  const progress = currentProgress.get(activity.id);
  const mastery = activityMasteryLabel(activity, progress);
  if (mastery.key === "retained") return { label: "Retained", className: "complete retained", icon: "◆" };
  if (mastery.key === "mastered") return { label: "Mastered", className: "complete", icon: "✓" };
  ...
  if (mastery.key === "repair") return { label: "Repair", className: "needs-work", icon: "!" };
  if (progress?.completed) return { label: "Completed · developing", className: "working", icon: "~" };
  if ((progress?.attempts || 0) > 0) return { label: "Developing", className: "needs-work", icon: "~" };
...
function acceleratorActivitiesForUnit(unitId) {
  const activities = visibleActivities().filter(activity => activity.unitId === unitId);
  const selected = [];
  const addFirstByType = types => {
    for (const type of types) {
      const item = activities.find(activity => activity.type === type && !selected.some(row => row.id === activity.id));
      if (item) {
        selected.push(item);
        return item;
      }
  ...
  const isPythonUnit = activities.some(activity => ["code", "debug", "visualiser"].includes(activity.type));
  const isSqlUnit = activities.some(activity => activity.type === "sql");
  const targetCount = isPythonUnit || isSqlUnit ? 5 : 4;

  addFirstByType(["lesson", "video"]);

  if (isPythonUnit) {
    // Every rapid Python unit must contain genuine coding practice. Code,
    // debug and visualiser activities all open the same integrated workspace:
    // Run, Check answer, Visualise, IDLE-style colours and smart indentation.
    addFirstByType(["code", "debug", "visualiser"]);
    addFirstByType(["code", "debug", "visualiser"]);
    addFirstByType(["fluency-sprint", "quiz", "predict"]);
    addFirstByType(["exam-style", "official-paper"]);
  } else if (isSqlUnit) {
    // SQL accelerator units use two live database laboratories where possible,
    // rather than replacing practical work with recognition-only questions.
    addFirstByType(["sql"]);
    addFirstByType(["sql"]);
    addFirstByType(["quiz", "fluency-sprint"]);
    addFirstByType(["exam-style", "official-paper"]);
  } else {
    addFirstByType(["binary-builder", "erd-builder", "code", "debug", "visualiser", "predict", "sql"]);
    addFirstByType(["quiz", "fluency-sprint"]);
    addFirstByType(["exam-style", "official-paper"]);
  }

  activities.filter(activity => activity.required !== false).forEach(activity => {
    if (selected.length < targetCount && !selected.some(row => row.id === activity.id)) selected.push(activity);
  });
  activities.forEach(activity => {
    if (selected.length < targetCount && !selected.some(row => row.id === activity.id)) selected.push(activity);
  });

  return selected.length ? selected.slice(0, targetCount) : activities.slice(0, targetCount);
}

function activitiesForUnit(unitId) {
  if (learningRoute === LEARNING_ROUTES.ACCELERATOR) return acceleratorActivitiesForUnit(unitId);
  ...
  AREAS.sort((a, b) => a.order - b.order).forEach(area => {
    ...
    card.innerHTML = `
      ...
      <div class="card-actions">
        <button ${area.active ? "" : "disabled"}>${area.active ? (learningRoute === LEARNING_ROUTES.ACCELERATOR ? "Open rapid route" : "Open pathway") : "Coming later"}</button>
      </div>`;
    if (area.active) {
...
function renderPathway() {
  ...
  units.forEach(unit => {
    ...
    const mastered = unitCore.filter(activity => masteryGateSatisfied(activity)).length;
    const completed = unitCore.filter(activity => currentProgress.get(activity.id)?.completed).length;
    const percent = unitCore.length ? Math.round((mastered / unitCore.length) * 100) : 0;
    ...
    unitCard.innerHTML = `
      <div class="unit-header">
        ...
        <div class="unit-progress">
          <span>${mastered}/${unitCore.length} mastered${completed > mastered ? ` · ${completed} completed` : ""}</span>
          <div class="progress-bar"><span style="width:${percent}%"></span></div>
        </div>
      </div>
    ...
    activities.forEach((activity, index) => {
      ...
      row.className = `activity-row ${unlocked ? "" : "locked"} ${activity.required === false ? "optional" : ""} ${activity.type === "fluency-sprint" ? "fluency-row" : ""}`;
      ...
      row.innerHTML = `
        ...
        <div class="activity-main">
          ...
          <div class="activity-meta">
            ...
            ${activity.type === "fluency-sprint" ? '<span class="activity-chip fluency-chip">6 practices</span>' : ""}
        ...
        <button ${unlocked ? "" : "disabled"}>${isActivityMastered(activity, progress) ? "Open again" : progress?.completed ? "Practise again" : progress ? "Continue" : activity.type === "official-paper" ? "Open paper task" : "Start"}</button>`;
...
function questionStatDocumentId(questionId) {
  return `${learningScopeId()}__${questionId}`;
}
...
async function loadSpacedLearningData() {
  ...
  const [stateSnapshot, attemptSnapshot, statSnapshot] = await Promise.all([
    ...
    getDocs(query(collection(db, "reviewQuestionStats"), where("classId", "==", learningScopeId())))
  ]);
  currentReviewStates = new Map(stateSnapshot.docs.map(item => [item.data().skillId, item.data()]));
  currentReviewAttempts = attemptSnapshot.docs.map(item => item.data()).filter(item => item.classId === learningScopeId());
...
function selectSpacedQuestions() {
  ...
  const seed = `${auth.currentUser?.uid || "pupil"}:${new Date().toISOString().slice(0, 10)}:spaced`;
  return selected.slice(0, 10).map((item, index) => ({ ...item, question: makeQuestionVariant(item.question, `${seed}:${index}`) }));
}
...
async function saveSpacedAnswer(question, selectedAnswer, confidence, correct, retry) {
  ...
  const statRef = doc(db, "reviewQuestionStats", questionStatDocumentId(sourceQuestionId));
  ...
  if (classStatisticSaved) {
    const oldStat = currentClassQuestionStats.get(sourceQuestionId) || { attempts: 0, correct: 0 };
    currentClassQuestionStats.set(sourceQuestionId, {
      ...oldStat,
...
function areaUnits(areaId) {
  return UNITS.filter(unit => unit.areaId === areaId).sort((a, b) => Number(a.order || 0) - Number(b.order || 0));
}

function placementForArea(areaId) {
  return currentPreferences?.masteryPlacement?.[areaId] || null;
}

function areaWorkingLevel(areaId) {
  const units = areaUnits(areaId);
  if (!units.length) return null;
  const placement = placementForArea(areaId);
  let bestIndex = placement?.unitId ? Math.max(0, units.findIndex(unit => unit.id === placement.unitId)) : 0;

  units.forEach((unit, index) => {
    const core = requiredActivities(areaId).filter(activity => activity.unitId === unit.id);
    if (!core.length) return;
    const secure = core.filter(activity => masteryGateSatisfied(activity)).length;
    const started = core.filter(activity => currentProgress.has(activity.id)).length;
    if (secure / core.length >= 0.5 || (secure > 0 && started >= Math.min(2, core.length))) bestIndex = Math.max(bestIndex, index);
  });

  const unit = units[Math.min(bestIndex, units.length - 1)];
  const meta = MASTERY_AREA_META[areaId];
  return {
    areaId,
    unitId: unit.id,
...
function renderMasteryLevelCards() {
  if (!elements.masteryLevelGrid) return;
  elements.masteryLevelGrid.innerHTML = ["sdd", "ddd", "cs"].map(areaId => {
    const level = areaWorkingLevel(areaId);
    const placement = placementForArea(areaId);
    return `<article class="card mastery-level-card area-${areaId}">
      <span class="mastery-level-icon">${level.icon}</span>
      <div><p class="eyebrow">${escapeHtml(level.name)} working level</p><h3>${escapeHtml(level.code)}</h3><p>${escapeHtml(level.title)}</p>
      <span class="activity-chip">${placement ? "Diagnostic + live evidence" : "Live pathway evidence"}</span></div>
    </article>`;
  }).join("");
}
...
function shuffledWithAnswer(options, answer, random) {
  const rows = options.map((option, index) => ({ option, correct: index === Number(answer) }));
  for (let i = rows.length - 1; i > 0; i -= 1) {
    const j = Math.floor(random() * (i + 1));
    [rows[i], rows[j]] = [rows[j], rows[i]];
  }
  return { options: rows.map(row => row.option), answer: rows.findIndex(row => row.correct) };
}

function surfaceVariantText(value, random) {
  let text = String(value ?? "");
  const groups = [
    [["Aberdeen", "Dundee", "Perth", "Inverness"], Math.floor(random() * 4)],
    [["Glasgow", "Stirling", "Edinburgh", "Paisley"], Math.floor(random() * 4)],
    [["Jones", "Patel", "Wilson", "Chen"], Math.floor(random() * 4)],
    [["Ava", "Maya", "Lewis", "Sam"], Math.floor(random() * 4)],
    [["Taylor", "Morgan", "Riley", "Jamie"], Math.floor(random() * 4)]
  ];
  for (const [items, index] of groups) {
    const replacement = items[index];
    items.forEach(original => { text = text.replace(new RegExp(`\b${original}\b`, "g"), replacement); });
  }
  return text;
}

function makeQuestionVariant(question, seed = `${Date.now()}`) {
  const random = seededRandom(`${question.id}:${seed}`);
  const sourceQuestionId = question.sourceQuestionId || question.id;
  if (["cs-binary-01", "cs-conversion-02"].includes(question.unitId)) {
    const value = 1 + Math.floor(random() * 254);
    const binary = value.toString(2).padStart(8, "0").replace(/(.{4})(.{4})/, "$1 $2");
    if (random() < 0.5) {
      const distractors = [...new Set([value, Math.max(0, value - 8), Math.min(255, value + 8), Math.max(0, value - 1)])].slice(0, 4);
      while (distractors.length < 4) distractors.push((value + distractors.length * 17) % 256);
      const answerIndex = distractors.indexOf(value);
      const shuffled = shuffledWithAnswer(distractors.map(String), answerIndex, random);
      return {
        ...question,
        id: `${sourceQuestionId}__variant_${Math.floor(random() * 1e9)}`,
        sourceQuestionId,
        ...
        explanation: `${binary} represents ${value}.`,
        skills: ["binary", "denary", "conversion"]
      };
    }
    const patterns = [value, value ^ 8, Math.min(255, value + 1), Math.max(0, value - 1)]
      .map(number => number.toString(2).padStart(8, "0").replace(/(.{4})(.{4})/, "$1 $2"));
    const shuffled = shuffledWithAnswer(patterns, 0, random);
    return {
      ...question,
      id: `${sourceQuestionId}__variant_${Math.floor(random() * 1e9)}`,
      sourceQuestionId,
      prompt: `Which 8-bit pattern represents denary ${value}?`,
      codeSnippet: "",
      diagramHtml: "",
      options: shuffled.options,
      answer: shuffled.answer,
      explanation: `${value} is ${binary}.`,
      skills: ["binary", "denary", "conversion"]
    };
  }

  const surfaceQuestion = question.areaId === "cs" ? question : {
    ...question,
    prompt: surfaceVariantText(question.prompt, random),
    codeSnippet: question.codeSnippet ? surfaceVariantText(question.codeSnippet, random) : question.codeSnippet,
    explanation: surfaceVariantText(question.explanation || "", random),
    options: (question.options || []).map(option => surfaceVariantText(option, random))
  };
  const shuffled = shuffledWithAnswer(surfaceQuestion.options || [], surfaceQuestion.answer, random);
  return {
    ...surfaceQuestion,
    id: `${sourceQuestionId}__variant_${Math.floor(random() * 1e9)}`,
    sourceQuestionId,
...
function masteryEligibleQuestions() {
  const placement = currentPreferences?.masteryPlacement || {};
  return SPACED_QUESTIONS.map(question => ({ ...question, areaId: spacedQuestionAreaId(question) })).filter(question => {
    const unit = unitById(question.unitId);
    if (!unit) return false;
    const placed = placement[unit.areaId];
    if (placed) {
      const placedUnit = unitById(placed.unitId);
      if (placedUnit && Number(unit.order || 0) <= Number(placedUnit.order || 0)) return true;
    }
    return allActivities.some(activity => activity.unitId === unit.id && masteryGateSatisfied(activity));
  });
}

function weakMasterySkillIds() {
  const weak = new Set();
  ownSkillAccuracy().forEach((row, skillId) => {
    if (Number(row.attempts || 0) >= 2 && Number(row.correct || 0) / Number(row.attempts || 1) < 0.7) weak.add(skillId);
  });
  currentReviewStates.forEach((state, skillId) => { if (state.lastCorrect === false) weak.add(skillId); });
  currentProgress.forEach((progress, activityId) => {
    if (Number(progress.incorrectStreak || 0) >= 2 || progress.masteryStatus === "repair") {
      const activity = allActivities.find(item => item.id === activityId);
      activitySkillIds(activity).forEach(skill => weak.add(skill));
    }
  });
  return weak;
}

function prerequisiteRepairActivity() {
  const failing = sortedActivities(requiredActivities()).filter(activity => {
    const progress = currentProgress.get(activity.id);
    return progress && (Number(progress.incorrectStreak || 0) >= 2 || progress.masteryStatus === "repair");
  }).at(-1);
  if (!failing) return null;
  const allRequired = requiredActivities(failing.areaId);
  const index = allRequired.findIndex(item => item.id === failing.id);
  const failingSkills = new Set(activitySkillIds(failing));
  const prior = allRequired.slice(0, Math.max(0, index)).reverse();
  const overlap = prior.find(activity => !INSTRUCTIONAL_ACTIVITY_TYPES.has(activity.type) && activitySkillIds(activity).some(skill => failingSkills.has(skill)));
  const fallback = prior.find(activity => !INSTRUCTIONAL_ACTIVITY_TYPES.has(activity.type));
  const repair = overlap || fallback;
  return repair ? { activity: repair, causedBy: failing } : { activity: failing, causedBy: failing };
}

function nextMasteryActivity() {
  const repair = prerequisiteRepairActivity();
  if (repair) return { ...repair, reason: "repair" };

  const areaOrder = ["sdd", "ddd", "cs"].sort((a, b) => {
    const aLevel = areaWorkingLevel(a)?.unitNumber || 1;
    const bLevel = areaWorkingLevel(b)?.unitNumber || 1;
    return aLevel - bLevel;
  });
  for (const areaId of areaOrder) {
    const candidate = requiredActivities(areaId).find(activity => !placementBypassesActivity(activity) && !isActivityMastered(activity) && isActivityUnlocked(activity));
    if (candidate) return { activity: candidate, causedBy: null, reason: currentProgress.get(candidate.id)?.completed ? "strengthen" : "new" };
  }
  return null;
}

function pickDistinctQuestions(pool, used, limit, predicate, reason) {
  const result = [];
  const candidates = personalQuestionSort(pool.filter(predicate));
  for (const question of candidates) {
    const sourceId = question.sourceQuestionId || question.id;
    if (used.has(sourceId)) continue;
    used.add(sourceId);
    result.push({ question, reason });
    if (result.length >= limit) break;
  }
  return result;
}

function buildDailyMasteryPlan() {
  const pool = masteryEligibleQuestions();
  const used = new Set();
  const weakSkills = weakMasterySkillIds();
  const now = Date.now();
  const selected = [];
  selected.push(...pickDistinctQuestions(pool, used, 2, q => questionStage(q) >= 3, "old"));
  selected.push(...pickDistinctQuestions(pool, used, 3, q => questionNextReview(q) > 0 && questionNextReview(q) <= now, "due"));
  selected.push(...pickDistinctQuestions(pool, used, 3, q => (q.skills || []).some(skill => weakSkills.has(skill)), "weak"));
  selected.push(...pickDistinctQuestions(pool, used, 8 - selected.length, () => true, "mixed"));

  const variantSeed = `${auth.currentUser?.uid || "pupil"}:${new Date().toISOString().slice(0, 10)}`;
  const questions = selected.slice(0, 8).map((item, index) => ({ ...item, question: makeQuestionVariant(item.question, `${variantSeed}:${index}`) }));
  const recommendation = nextMasteryActivity();
  const minutes = Math.max(8, Math.min(15, Math.round(questions.length * 0.9 + Number(recommendation?.activity?.estimatedMinutes || 4))));
  return { questions, recommendation, minutes, weakSkills };
}

const DAILY_REASON_LABELS = Object.freeze({ old: "Older mastery", due: "Due retrieval", weak: "Strengthen", mixed: "Interleaved", new: "New / next step" });

function renderDailyMasteryPlan() {
  renderMasteryLevelCards();
  const plan = buildDailyMasteryPlan();
  dailyMasterySession = { ...plan, index: 0, correct: 0, answered: 0, results: [] };
  elements.dailyMasteryTimeBadge.textContent = `${plan.minutes} min`;
  elements.dailyMasteryPlanTitle.textContent = plan.questions.length ? "Today's prescribed practice" : "Take a starting diagnostic";
  elements.dailyMasteryPlanDescription.textContent = plan.questions.length
    ? "You do not need to choose a topic. The system has mixed older learning, due retrieval and weak skills, then chosen the best next activity."
    : "There is not enough evidence to prescribe practice yet. Take a diagnostic in at least one area to set a starting level.";
  const counts = { old: 0, due: 0, weak: 0, mixed: 0 };
  plan.questions.forEach(item => { counts[item.reason] = Number(counts[item.reason] || 0) + 1; });
  const oldAndInterleaved = counts.old + counts.mixed;
  elements.dailyMasteryMix.innerHTML = [
    ["🔁", oldAndInterleaved, "old"], ["⏰", counts.due, "due"], ["🛠", counts.weak, "weak"], ["🚀", plan.recommendation?.activity ? 1 : 0, "new"]
  ].map(([icon, value, key]) => `<div><span>${icon}</span><strong>${value}</strong><small>${DAILY_REASON_LABELS[key]}</small></div>`).join("");

  if (plan.recommendation?.activity) {
    const rec = plan.recommendation;
    const unit = unitById(rec.activity.unitId);
    const label = rec.reason === "repair" ? "Prerequisite repair" : rec.reason === "strengthen" ? "Strengthen before moving on" : "New learning";
    elements.dailyMasteryRecommendation.innerHTML = `<span class="activity-chip">${escapeHtml(label)}</span><strong>${escapeHtml(rec.activity.title)}</strong><small>${escapeHtml(unit?.title || "")}${rec.causedBy && rec.causedBy.id !== rec.activity.id ? ` · selected because ${escapeHtml(rec.causedBy.title)} is showing repeated errors` : ""}</small>`;
  } else {
    elements.dailyMasteryRecommendation.innerHTML = `<span class="activity-chip">Consolidation</span><strong>No new activity needed today</strong><small>Use the retrieval set to maintain long-term mastery.</small>`;
  }
  elements.startDailyMasteryButton.disabled = plan.questions.length === 0;
  elements.startDailyMasteryButton.textContent = plan.questions.length ? "Start today's mastery" : "Take a diagnostic first";
  elements.dailyMasteryQuestionCard.classList.add("hidden");
  elements.dailyMasterySummaryCard.classList.add("hidden");
  elements.dailyMasteryPlanCard.classList.remove("hidden");

  const placements = currentPreferences?.masteryPlacement || {};
  const completed = ["sdd", "ddd", "cs"].filter(areaId => placements[areaId]).length;
  elements.masteryDiagnosticSummary.textContent = completed
    ? `${completed}/3 area diagnostics completed. Working levels continue to adjust using live mastery evidence.`
    : "A short adaptive check places Python, SQL and Computer Systems independently. You can retake any area later.";
}

async function openDailyMastery() {
  clearMessage();
  await loadSpacedLearningData();
  renderDailyMasteryPlan();
  showView("dailyMasteryView");
  window.scrollTo({ top: 0, behavior: "smooth" });
}

function renderDailyMasteryQuestion() {
  const item = dailyMasterySession?.questions?.[dailyMasterySession.index];
  if (!item) return void finishDailyMasterySession();
  const q = item.question;
  const area = areaById(spacedQuestionAreaId(q));
  elements.dailyMasteryProgress.textContent = `${dailyMasterySession.index + 1}/${dailyMasterySession.questions.length}`;
  elements.dailyMasteryQuestionTitle.textContent = DAILY_REASON_LABELS[item.reason] || "Mixed retrieval";
  elements.dailyMasteryQuestionMeta.innerHTML = `<span class="area-badge area-${escapeHtml(area.id)}">${escapeHtml(area.shortTitle)}</span><span class="activity-chip">${escapeHtml(spacedQuestionModuleLabel(q))}</span><span class="activity-chip reason-chip">${escapeHtml(DAILY_REASON_LABELS[item.reason] || item.reason)}</span>`;
  elements.dailyMasteryQuestionPrompt.textContent = q.prompt;
  elements.dailyMasteryQuestionOptions.innerHTML = (q.options || []).map((option, index) => `<label class="option-label"><input type="radio" name="daily-mastery-answer" value="${index}"> <span>${escapeHtml(option)}</span></label>`).join("");
  if (q.diagramHtml) {
    elements.dailyMasteryQuestionCode.classList.remove("hidden");
    elements.dailyMasteryQuestionCode.innerHTML = `<div class="spaced-diagram">${q.diagramHtml}</div>`;
  } else if (q.codeSnippet) {
    elements.dailyMasteryQuestionCode.classList.remove("hidden");
    elements.dailyMasteryQuestionCode.innerHTML = `<pre class="lesson-code">${escapeHtml(String(q.codeSnippet).replace(/\\n/g, "\n"))}</pre>`;
  } else {
    elements.dailyMasteryQuestionCode.classList.add("hidden");
    elements.dailyMasteryQuestionCode.innerHTML = "";
  }
  elements.dailyMasteryConfidenceInput.value = "2";
  elements.submitDailyMasteryAnswerButton.disabled = false;
  elements.submitDailyMasteryAnswerButton.classList.remove("hidden");
  elements.nextDailyMasteryQuestionButton.classList.add("hidden");
  elements.dailyMasteryFeedback.className = "feedback hidden";
  spacedQuestionStartedAt = Date.now();
}

async function submitDailyMasteryAnswer() {
  const item = dailyMasterySession?.questions?.[dailyMasterySession.index];
  if (!item || item.answered) return;
  const selected = elements.dailyMasteryQuestionOptions.querySelector('input[name="daily-mastery-answer"]:checked');
  if (!selected) {
    elements.dailyMasteryFeedback.textContent = "Choose an answer before checking.";
    elements.dailyMasteryFeedback.className = "feedback error";
    return;
  }
  const answer = Number(selected.value);
  const confidence = Number(elements.dailyMasteryConfidenceInput.value || 2);
  const correct = answer === Number(item.question.answer);
  elements.submitDailyMasteryAnswerButton.disabled = true;
  try {
    await saveSpacedAnswer(item.question, answer, confidence, correct, false);
    item.answered = true;
    item.correct = correct;
    dailyMasterySession.answered += 1;
    if (correct) dailyMasterySession.correct += 1;
    dailyMasterySession.results.push({ question: item.question, reason: item.reason, correct });
    elements.dailyMasteryFeedback.innerHTML = correct
      ? `<strong>Correct.</strong> ${escapeHtml(item.question.explanation || "")}`
      : `<strong>Not yet.</strong> The correct answer is <strong>${escapeHtml(item.question.options[item.question.answer])}</strong>. ${escapeHtml(item.question.explanation || "")} This skill has been moved closer in your mastery schedule.`;
    elements.dailyMasteryFeedback.className = `feedback ${correct ? "success" : "error"}`;
    elements.submitDailyMasteryAnswerButton.classList.add("hidden");
    elements.nextDailyMasteryQuestionButton.classList.remove("hidden");
    elements.nextDailyMasteryQuestionButton.textContent = dailyMasterySession.index >= dailyMasterySession.questions.length - 1 ? "Finish retrieval →" : "Next question →";
  } catch (error) {
    elements.submitDailyMasteryAnswerButton.disabled = false;
    elements.dailyMasteryFeedback.textContent = error.message || "The answer could not be saved.";
    elements.dailyMasteryFeedback.className = "feedback error";
  }
}

async function finishDailyMasterySession() {
  if (!dailyMasterySession) return;
  const percentage = dailyMasterySession.answered ? Math.round(dailyMasterySession.correct / dailyMasterySession.answered * 100) : 0;
  try {
    await setDoc(doc(collection(db, "reviewSessions")), {
      userId: auth.currentUser.uid,
      classId: learningScopeId(),
      courseId: COURSE_ID,
      sessionType: "daily-mastery",
      masteryVersion: MASTERY_VERSION,
      mainQuestions: dailyMasterySession.answered,
      correct: dailyMasterySession.correct,
      retryQuestions: 0,
      percentage,
      recommendationActivityId: dailyMasterySession.recommendation?.activity?.id || null,
      recommendationReason: dailyMasterySession.recommendation?.reason || "consolidate",
      completedAt: serverTimestamp()
    });
  } catch (error) {
    console.warn("Daily Mastery session summary could not be saved:", error);
  }
  await loadSpacedLearningData();
  const weakAfter = weakMasterySkillIds();
  const retained = [...currentReviewStates.values()].filter(state => Number(state.stage || 0) >= 3 && state.lastCorrect !== false).length;
  const rec = dailyMasterySession.recommendation;
  elements.dailyMasteryQuestionCard.classList.add("hidden");
  elements.dailyMasteryPlanCard.classList.add("hidden");
  elements.dailyMasterySummaryCard.classList.remove("hidden");
  elements.dailyMasterySummaryCard.innerHTML = `<div class="spaced-result-hero"><span class="spaced-result-score">${percentage}%</span><div><p class="eyebrow">Retrieval complete</p><h3>${dailyMasterySession.correct}/${dailyMasterySession.answered} first-pass correct</h3><p>${retained} skills currently show retained evidence · ${weakAfter.size} skills are scheduled for strengthening.</p></div></div>
    ${rec?.activity ? `<div class="mastery-next-step"><span class="activity-chip">${rec.reason === "repair" ? "Repair next" : rec.reason === "strengthen" ? "Strengthen next" : "New learning"}</span><h3>${escapeHtml(rec.activity.title)}</h3><p>${escapeHtml(unitById(rec.activity.unitId)?.title || "")}</p></div>` : `<p>Your learning is consolidated for today. Return tomorrow for a fresh prescription.</p>`}
    <div class="activity-actions">${rec?.activity ? '<button id="openDailyRecommendedActivityButton" type="button">Continue to recommended activity →</button>' : ''}<button id="finishDailyMasteryButton" type="button" class="secondary">Return to dashboard</button></div>`;
  elements.dailyMasterySummaryCard.querySelector("#openDailyRecommendedActivityButton")?.addEventListener("click", () => openActivity(rec.activity));
  elements.dailyMasterySummaryCard.querySelector("#finishDailyMasteryButton")?.addEventListener("click", async () => { showView("pupilView"); await loadPupilDashboard(); });
}

function diagnosticPoolForArea(areaId) {
  return SPACED_QUESTIONS.map(q => ({ ...q, areaId: spacedQuestionAreaId(q) })).filter(q => q.areaId === areaId && (q.options || []).length >= 2);
}

function chooseDiagnosticQuestion() {
  const session = diagnosticSession;
  const available = session.pool.filter(q => !session.used.has(q.id));
  if (!available.length) return null;
  available.sort((a, b) => {
    const da = Math.abs(Number(unitById(a.unitId)?.order || 0) - session.targetOrder);
    const db = Math.abs(Number(unitById(b.unitId)?.order || 0) - session.targetOrder);
    return da - db;
  });
  const top = available.slice(0, Math.min(4, available.length));
  const selected = top[Math.floor(Math.random() * top.length)];
  session.used.add(selected.id);
  return makeQuestionVariant(selected, `diagnostic:${session.areaId}:${session.answered}:${Date.now()}`);
}

function renderDiagnosticQuestion() {
  if (!diagnosticSession) return;
  if (diagnosticSession.answered >= diagnosticSession.targetQuestions) return void finishDiagnostic();
  const q = chooseDiagnosticQuestion();
  if (!q) return void finishDiagnostic();
  diagnosticSession.current = q;
  const unit = unitById(q.unitId);
  elements.diagnosticProgress.textContent = `${diagnosticSession.answered + 1}/${diagnosticSession.targetQuestions}`;
  elements.diagnosticQuestionMeta.innerHTML = `<span class="activity-chip">Adaptive</span><span class="activity-chip">${escapeHtml(unit?.title || "")}</span>`;
  elements.diagnosticQuestionPrompt.textContent = q.prompt;
  elements.diagnosticQuestionOptions.innerHTML = q.options.map((option, index) => `<label class="option-label"><input type="radio" name="diagnostic-answer" value="${index}"> <span>${escapeHtml(option)}</span></label>`).join("");
  if (q.diagramHtml) {
    elements.diagnosticQuestionCode.classList.remove("hidden");
    elements.diagnosticQuestionCode.innerHTML = `<div class="spaced-diagram">${q.diagramHtml}</div>`;
  } else if (q.codeSnippet) {
    elements.diagnosticQuestionCode.classList.remove("hidden");
    elements.diagnosticQuestionCode.innerHTML = `<pre class="lesson-code">${escapeHtml(String(q.codeSnippet).replace(/\\n/g, "\n"))}</pre>`;
  } else {
    elements.diagnosticQuestionCode.classList.add("hidden"); elements.diagnosticQuestionCode.innerHTML = "";
  }
  elements.diagnosticFeedback.className = "feedback hidden";
  elements.submitDiagnosticAnswerButton.classList.remove("hidden");
  elements.submitDiagnosticAnswerButton.disabled = false;
  elements.nextDiagnosticQuestionButton.classList.add("hidden");
}

function openMasteryDiagnostic(areaId) {
  const units = areaUnits(areaId);
  const pool = diagnosticPoolForArea(areaId);
  if (!pool.length) { showMessage("No diagnostic questions are available for this area yet.", "error"); return; }
  const startOrder = Math.max(1, Math.round(units.length * 0.35));
  diagnosticSession = { areaId, units, pool, used: new Set(), answered: 0, correct: 0, targetQuestions: Math.min(8, pool.length), targetOrder: startOrder, successfulOrders: [], current: null };
  elements.diagnosticTitle.textContent = `${MASTERY_AREA_META[areaId].name} starting diagnostic`;
  elements.diagnosticSummaryPanel.classList.add("hidden");
  elements.diagnosticQuestionPanel.classList.remove("hidden");
  showView("masteryDiagnosticView");
  renderDiagnosticQuestion();
  window.scrollTo({ top: 0, behavior: "smooth" });
}

async function submitDiagnosticAnswer() {
  if (!diagnosticSession?.current) return;
  const selected = elements.diagnosticQuestionOptions.querySelector('input[name="diagnostic-answer"]:checked');
  if (!selected) { elements.diagnosticFeedback.textContent = "Choose an answer first."; elements.diagnosticFeedback.className = "feedback error"; return; }
  const q = diagnosticSession.current;
  const correct = Number(selected.value) === Number(q.answer);
  const order = Number(unitById(q.unitId)?.order || 1);
  diagnosticSession.answered += 1;
  if (correct) { diagnosticSession.correct += 1; diagnosticSession.successfulOrders.push(order); }
  const maxOrder = diagnosticSession.units.length;
  const distanceUp = Math.max(1, Math.round((maxOrder - diagnosticSession.targetOrder) / 3));
  const distanceDown = Math.max(1, Math.round(diagnosticSession.targetOrder / 3));
  diagnosticSession.targetOrder = Math.max(1, Math.min(maxOrder, diagnosticSession.targetOrder + (correct ? distanceUp : -distanceDown)));
  elements.diagnosticFeedback.innerHTML = correct ? `<strong>Correct.</strong> The next question will move a little higher.` : `<strong>Not yet.</strong> The answer is <strong>${escapeHtml(q.options[q.answer])}</strong>. The next question will adjust down.`;
  elements.diagnosticFeedback.className = `feedback ${correct ? "success" : "error"}`;
  elements.submitDiagnosticAnswerButton.classList.add("hidden");
  elements.nextDiagnosticQuestionButton.classList.remove("hidden");
  elements.nextDiagnosticQuestionButton.textContent = diagnosticSession.answered >= diagnosticSession.targetQuestions ? "See my starting level →" : "Next adaptive question →";
}

async function finishDiagnostic() {
  if (!diagnosticSession) return;
  const session = diagnosticSession;
  const maxSuccess = session.successfulOrders.length ? Math.max(...session.successfulOrders) : 1;
  const placementOrder = Math.max(1, Math.min(session.units.length, Math.round((maxSuccess + session.targetOrder) / 2)));
  const unit = session.units[Math.max(0, placementOrder - 1)] || session.units[0];
  const meta = MASTERY_AREA_META[session.areaId];
  const placement = {
    ...(currentPreferences.masteryPlacement || {}),
    [session.areaId]: { unitId: unit.id, unitNumber: unit.number, score: session.correct, answered: session.answered, setAt: new Date().toISOString(), version: MASTERY_VERSION }
  };
  await setDoc(doc(db, "preferences", auth.currentUser.uid), { userId: auth.currentUser.uid, masteryPlacement: placement, updatedAt: serverTimestamp() }, { merge: true });
  const placementTaskId = `MASTERY-PLACEMENT-${session.areaId}`;
  const placementProgress = {
    classId: learningScopeId(),
    courseId: COURSE_ID,
    userId: auth.currentUser.uid,
    taskId: placementTaskId,
    activityId: placementTaskId,
    taskTitle: `${meta.name} adaptive starting diagnostic`,
    activityTitle: `${meta.name} adaptive starting diagnostic`,
    activityType: "mastery-diagnostic",
    areaId: session.areaId,
    unitId: unit.id,
    placementUnitId: unit.id,
    placementUnitNumber: unit.number,
    placementScore: session.correct,
    placementQuestions: session.answered,
    completed: true,
    mastered: true,
    masteryStatus: "placement",
    masteryVersion: MASTERY_VERSION,
    lastActivityAt: serverTimestamp(),
    lastSavedAt: serverTimestamp()
  };
  await setDoc(doc(db, "progress", progressDocumentId(placementTaskId)), placementProgress, { merge: true });
  currentProgress.set(placementTaskId, placementProgress);
  currentPreferences = { ...currentPreferences, masteryPlacement: placement };
  elements.diagnosticQuestionPanel.classList.add("hidden");
  elements.diagnosticSummaryPanel.classList.remove("hidden");
  elements.diagnosticProgress.textContent = `${session.correct}/${session.answered}`;
  elements.diagnosticSummaryPanel.innerHTML = `<div class="diagnostic-result"><span class="mastery-level-icon">${meta.icon}</span><div><p class="eyebrow">Starting level</p><h2>${meta.prefix}${unit.number}</h2><h3>${escapeHtml(unit.title)}</h3><p>${session.correct}/${session.answered} diagnostic questions correct. This is a comfortable starting point, not a ceiling; live mastery evidence will move the level as the pupil progresses.</p></div></div><div class="activity-actions"><button id="returnToDailyAfterDiagnosticButton" type="button">Build my Daily Mastery plan</button></div>`;
  elements.diagnosticSummaryPanel.querySelector("#returnToDailyAfterDiagnosticButton").addEventListener("click", () => void openDailyMastery());
}

function renderLicenceStatus() {
  if (!elements.licenceStatusCard) return;
  const label = licenceLabel();
  elements.licenceStatusTitle.textContent = label;
  if (currentEntitlement?.permanent === true) {
    elements.licenceStatusText.textContent = "Full Course, Daily Mastery and Exam Accelerator are permanently available on this account.";
    elements.licenceStatusBadge.textContent = "Permanent";
  } else if (hasSchoolClass()) {
    elements.licenceStatusText.textContent = "Your access is provided through your class. Your progress is saved to this pupil account.";
    elements.licenceStatusBadge.textContent = "Active";
  } else {
    elements.licenceStatusText.textContent = "Full Course, Daily Mastery and Exam Accelerator are available through your individual licence.";
    elements.licenceStatusBadge.textContent = "Active";
  }
}

function renderLearningRouteControls() {
  if (!elements.fullCourseRouteButton) return;
  const accelerator = learningRoute === LEARNING_ROUTES.ACCELERATOR;
  elements.fullCourseRouteButton.classList.toggle("active", !accelerator);
  elements.examAcceleratorRouteButton.classList.toggle("active", accelerator);
  elements.learningRouteNotice.classList.toggle("accelerator", accelerator);
  elements.acceleratorMethodPanel?.classList.toggle("hidden", !accelerator);
  elements.pupilView.classList.toggle("accelerator-route", accelerator);
  elements.learningRouteNotice.innerHTML = accelerator
    ? "<strong>Exam Accelerator:</strong> every topic remains available. Python units include practical coding in the combined Run + Visualise IDLE-style editor, and SQL units include live relational database laboratories. Progress continues to count towards the same mastery profile."
    : "<strong>Full Course:</strong> work through the carefully sequenced pathway. New assessed activities unlock when prerequisite skills are mastered enough to advance, while teaching explanations remain available to revisit.";
}

async function changeLearningRoute(route) {
  if (!Object.values(LEARNING_ROUTES).includes(route) || route === learningRoute) return;
  learningRoute = route;
  renderLearningRouteControls();
  elements.pathwayPanel.classList.add("hidden");
  elements.unitList.innerHTML = "";
  renderAreaCards();
  try { await saveLearningRoutePreference(); }
  catch (error) { console.warn("Learning route preference could not be saved:", error); }
}

elements.fullCourseRouteButton?.addEventListener("click", () => changeLearningRoute(LEARNING_ROUTES.FULL));
elements.examAcceleratorRouteButton?.addEventListener("click", () => changeLearningRoute(LEARNING_ROUTES.ACCELERATOR));
elements.dailyMasteryRouteButton?.addEventListener("click", () => void openDailyMastery());
elements.backFromDailyMasteryButton?.addEventListener("click", async () => { showView("pupilView"); await loadPupilDashboard(); });
elements.rebuildDailyMasteryButton?.addEventListener("click", () => renderDailyMasteryPlan());
elements.startDailyMasteryButton?.addEventListener("click", () => {
  if (!dailyMasterySession?.questions?.length) return;
  elements.dailyMasteryPlanCard.classList.add("hidden");
  elements.dailyMasteryQuestionCard.classList.remove("hidden");
  renderDailyMasteryQuestion();
});
elements.submitDailyMasteryAnswerButton?.addEventListener("click", () => void submitDailyMasteryAnswer());
elements.nextDailyMasteryQuestionButton?.addEventListener("click", () => {
  if (!dailyMasterySession) return;
  dailyMasterySession.index += 1;
  if (dailyMasterySession.index >= dailyMasterySession.questions.length) void finishDailyMasterySession();
  else renderDailyMasteryQuestion();
});
elements.diagnosticPythonButton?.addEventListener("click", () => openMasteryDiagnostic("sdd"));
elements.diagnosticSqlButton?.addEventListener("click", () => openMasteryDiagnostic("ddd"));
elements.diagnosticCsButton?.addEventListener("click", () => openMasteryDiagnostic("cs"));
elements.backFromMasteryDiagnosticButton?.addEventListener("click", () => void openDailyMastery());
elements.submitDiagnosticAnswerButton?.addEventListener("click", () => void submitDiagnosticAnswer());
elements.nextDiagnosticQuestionButton?.addEventListener("click", () => {
  if (!diagnosticSession) return;
  if (diagnosticSession.answered >= diagnosticSession.targetQuestions) void finishDiagnostic();
...
async function loadPupilDashboard() {
  ...
  elements.pupilClassLabel.textContent = hasSchoolClass() ? currentProfile.className : "Individual study account";
  renderLicenceStatus();
  renderLearningRouteControls();

  const coreActivities = requiredActivities();
  const coreIds = new Set(coreActivities.map(activity => activity.id));
  const completed = [...currentProgress.entries()].filter(([id, item]) => item.completed && coreIds.has(id)).length;
  const mastered = coreActivities.filter(activity => masteryGateSatisfied(activity)).length;
  const developing = coreActivities.filter(activity => {
    const progress = currentProgress.get(activity.id);
    return progress?.completed && !masteryGateSatisfied(activity);
  }).length;
  const attempts = [...currentProgress.values()].reduce((sum, item) => sum + (item.attempts || 0), 0);
  const retainedSkills = [...currentReviewStates.values()].filter(state => Number(state.stage || 0) >= 3 && Number(state.consecutiveCorrect || 0) >= 2 && state.lastCorrect !== false).length;
  const coreCount = coreIds.size;
  const percent = coreCount ? Math.round((mastered / coreCount) * 100) : 0;
  const pythonLevel = areaWorkingLevel("sdd");
  const sqlLevel = areaWorkingLevel("ddd");
  const csLevel = areaWorkingLevel("cs");
  elements.pupilProgressBadge.textContent = `${mastered}/${coreCount} required mastered`;
  elements.topicSummary.innerHTML = [
    statCard(`${percent}%`, "Whole-course mastery"),
    statCard(pythonLevel.code, "Python working level"),
    statCard(sqlLevel.code, "SQL working level"),
    statCard(csLevel.code, "CS working level"),
    statCard(completed, "Required completed"),
    statCard(developing, "Completed · developing"),
    statCard(retainedSkills, "Retained skills"),
    statCard(attempts, "Attempts")
  ].join("");

  renderSpacedPracticeCard();
  renderRevisionHubCard();
  renderGamesLabCard();
  if (hasSchoolClass()) await loadOrClaimPeerReview();
  else { activePeerReview = null; elements.peerMarkingCard.classList.add("hidden"); }
  renderAreaCards();
  elements.pathwayPanel.classList.add("hidden");
  elements.unitList.innerHTML = "";
}
...
const PYTHON_BUILTINS = new Set([
  "abs", "all", "any", "bool", "chr", "dict", "enumerate", "float", "input", "int",
  "len", "list", "max", "min", "open", "ord", "print", "range", "round", "set",
  "sorted", "str", "sum", "tuple", "type", "zip"
]);

function pythonHighlightHtml(code) {
  ...
  for (const match of source.matchAll(tokenPattern)) {
    ...
    const token = match[0];
    let className = "";
    if (token.startsWith("#")) className = "idle-comment";
    else if (token.startsWith("'") || token.startsWith('"')) className = "idle-string";
    else if (/^\d/.test(token)) className = "idle-number";
    else if (PYTHON_KEYWORDS.has(token)) className = "idle-keyword";
    else if (PYTHON_BUILTINS.has(token) || token === "random") className = "idle-builtin";
...
function refreshCodeHighlight(textarea) {
  if (!textarea) return;
  const shell = textarea.closest(".code-editor-shell");
  const highlight = shell?.querySelector(".code-highlight");
  if (!shell || !highlight) return;
  const language = textarea.dataset.editorLanguage || "python";
  if (language !== "python") {
    shell.classList.remove("syntax-ready");
    highlight.textContent = "";
    return;
  }
  highlight.innerHTML = pythonHighlightHtml(textarea.value);
  highlight.scrollTop = textarea.scrollTop;
  highlight.scrollLeft = textarea.scrollLeft;
  shell.classList.add("syntax-ready");
}

function setCodeEditorLanguage(textarea, language = "python") {
  if (!textarea) return;
  textarea.dataset.editorLanguage = language;
  textarea.setAttribute("aria-label", language === "python" ? "Python code editor" : "Answer editor");
  refreshCodeHighlight(textarea);
}

function insertTextAtSelection(textarea, replacement, selectionOffset = replacement.length) {
...
function handleSmartEditorKeydown(event) {
  ...
  if (event.key === "Tab") {
    ...
    if (event.shiftKey) {
      ...
      const unindented = selected.replace(/^ {1,4}/gm, match => "".padEnd(Math.max(0, match.length - 4), " "));
      textarea.value = textarea.value.slice(0, lineStart) + unindented + textarea.value.slice(end);
      textarea.selectionStart = lineStart;
      textarea.selectionEnd = lineStart + unindented.length;
      textarea.dispatchEvent(new Event("input", { bubbles: true }));
    } else if (start !== end && textarea.value.slice(start, end).includes("\n")) {
      const lineStart = textarea.value.lastIndexOf("\n", start - 1) + 1;
      const selected = textarea.value.slice(lineStart, end);
      const indented = selected.replace(/^/gm, "    ");
      textarea.value = textarea.value.slice(0, lineStart) + indented + textarea.value.slice(end);
      textarea.selectionStart = lineStart;
  ...
  if (event.key === "Enter" && language === "python") {
    event.preventDefault();
    const start = textarea.selectionStart;
    const lineStart = textarea.value.lastIndexOf("\n", start - 1) + 1;
    const beforeCursor = textarea.value.slice(lineStart, start);
...
function enhanceCodeTextarea(textarea, language = "python") {
  if (!textarea || textarea.dataset.smartEditorReady === "true") {
    if (textarea) setCodeEditorLanguage(textarea, language);
    return textarea;
  }
  let shell = textarea.closest(".code-editor-shell");
  if (!shell) {
  ...
  textarea.dataset.smartEditorReady = "true";
  textarea.classList.add("smart-code-input");
  textarea.setAttribute("wrap", "off");
  textarea.addEventListener("keydown", handleSmartEditorKeydown);
  textarea.addEventListener("input", () => refreshCodeHighlight(textarea));
  textarea.addEventListener("scroll", () => refreshCodeHighlight(textarea));
  setCodeEditorLanguage(textarea, language);
  return textarea;
}

function inferFluencyLanguage(task) {
  const combined = `${task?.answer || ""}\n${task?.starter || ""}`;
  const prompt = String(task?.prompt || "");
  if (task?.format !== "code") return "text";
  if (/\b(RECEIVE|SEND|SET|END IF|END WHILE|START FIXED LOOP|START CONDITIONAL LOOP|REPEAT|UNTIL)\b/.test(combined)) return "pseudocode";
  if (/pseudocode/i.test(prompt) && !/(Python|program|code)/i.test(prompt)) return "pseudocode";
  return "python";
}

function inferFluencySampleInputs(code) {
  const source = String(code || "");
  const calls = [...source.matchAll(/input\s*\(/g)].length;
  if (!calls) return [];
  const numeric = /(?:int|float)\s*\(\s*input\s*\(/.test(source);
  return Array.from({ length: calls }, (_, index) => numeric ? String(index + 3) : (index ? "Taylor" : "Ava"));
}

function fluencyProgressState(activity) {
  const saved = currentProgress.get(activity.id)?.fluency || {};
  const length = activity.tasks?.length || 6;
  return {
    index: Math.min(Number(saved.index || 0), Math.max(0, length - 1)),
    responses: Array.isArray(saved.responses) ? [...saved.responses] : Array(length).fill(""),
    correct: Array.isArray(saved.correct) ? [...saved.correct] : Array(length).fill(false),
    checkCounts: Array.isArray(saved.checkCounts) ? [...saved.checkCounts] : Array(length).fill(0),
    firstPassCorrect: Array.isArray(saved.firstPassCorrect) ? [...saved.firstPassCorrect] : Array(length).fill(null),
    hintsByPractice: Array.isArray(saved.hintsByPractice) ? [...saved.hintsByPractice] : Array(length).fill(0),
    secondsByPractice: Array.isArray(saved.secondsByPractice) ? [...saved.secondsByPractice] : Array(length).fill(0),
    startedAt: Date.now()
  };
}

function fluencyMasterySummary(state) {
  const firstPass = (state.firstPassCorrect || []).filter(value => value === true).length;
  const hints = (state.hintsByPractice || []).reduce((sum, value) => sum + Number(value || 0), 0);
  const seconds = (state.secondsByPractice || []).reduce((sum, value) => sum + Number(value || 0), 0);
  const allCorrect = (state.correct || []).every(Boolean);
  const mastered = Boolean(allCorrect && firstPass >= 5 && hints <= 1);
  return { firstPass, hints, seconds, allCorrect, mastered };
}

async function saveActiveFluencyProgress({ complete = false, countAttempt = false, correct = false } = {}) {
  if (!activeFluencyChallenge) return;
  const { activity, state } = activeFluencyChallenge;
  state.responses[state.index] = elements.codeEditor.value;
  const old = currentProgress.get(activity.id) || {};
  const summary = fluencyMasterySummary(state);
  const updates = {
    fluency: state,
    completed: Boolean(old.completed || complete),
    mastered: complete ? summary.mastered : old.mastered,
    masteryStatus: complete ? (summary.mastered ? "mastered" : "developing") : old.masteryStatus,
    fluencyFirstPassCorrect: summary.firstPass,
    fluencyHintsUsed: summary.hints,
    fluencySeconds: summary.seconds,
    lastCode: elements.codeEditor.value,
    lastFluencyAttempt: state.index + 1
  };
  if (countAttempt) {
    updates.attempts = (old.attempts || 0) + 1;
    updates.incorrectAttempts = (old.incorrectAttempts || 0) + (correct ? 0 : 1);
    updates.firstAttemptCorrect = (old.attempts || 0) === 0 ? Boolean(correct) : Boolean(old.firstAttemptCorrect);
  }
  await saveGenericProgress(activity, updates, { countAttempt, correct });
  codeIsDirty = false;
  clearScheduledAutoSave();
...
function renderSddFluencyAttempt() {
  if (!activeFluencyChallenge) return;
  const { activity, state } = activeFluencyChallenge;
  const tasks = activity.tasks || [];
  const task = tasks[state.index];
  const language = inferFluencyLanguage(task);
  const unit = unitById(activity.unitId);
  const support = state.index <= 1 ? "Worked pattern available" : state.index <= 3 ? "Compact reminder" : state.index === 4 ? "Generate from the prompt" : "Near transfer";
  const dots = tasks.map((_, index) => `<span class="${state.correct[index] ? "complete" : ""}${index === state.index ? " current" : ""}">${index + 1}</span>`).join("");
  const response = state.responses[state.index] || task.starter || "";
  const sampleInputs = language === "python" ? inferFluencySampleInputs(response || task.answer) : [];
  ...
  currentTask = {
    ...
    expectedOutput: language === "python" ? "Run your program to inspect its output." : "This is a written design response.",
    sampleInputs,
    hints: [task.hint || (state.index <= 1 ? `Pattern: ${task.answer}` : "Use the same structure as the previous successful attempt.")]
  };
  ...
  elements.challengeTopic.textContent = `Guided practice · Unit ${unit?.number || ""} ${unit?.title || ""}`;
  elements.challengeTitle.textContent = activity.title;
  elements.challengeInstructions.innerHTML = `
    <div class="fluency-progress-strip">${dots}</div>
    <div class="activity-meta"><span class="activity-chip">Practice ${state.index + 1} of ${tasks.length}</span><span class="activity-chip">${support}</span></div>
    <p class="fluency-challenge-prompt"><strong>${escapeHtml(task.prompt)}</strong></p>
    ${language === "python" ? '<p class="editor-help-line">Type Python below. Run and Visualise are available before you check your answer.</p>' : `<p class="editor-help-line">${language === "pseudocode" ? "Type the pseudocode below. Pseudocode cannot be executed by Python, so Run and Visualise are paused for this practice." : "Type your written answer below."}</p>`}`;
  elements.expectedOutput.textContent = currentTask.expectedOutput;
  elements.codeEditor.value = response;
  enhanceCodeTextarea(elements.codeEditor, language);
  refreshCodeHighlight(elements.codeEditor);
  elements.challengeStatus.textContent = state.correct[state.index] ? "Practice complete" : "In progress";
  elements.challengeStatus.className = `status-pill ${state.correct[state.index] ? "complete" : "working"}`;
  if (sampleInputs.length) {
    elements.sampleInputNote.textContent = `Run and Visualise will use sample keyboard input: ${sampleInputs.join(", ")}.`;
  ...
  elements.runButton.disabled = language !== "python";
  elements.visualiseButton.disabled = language !== "python";
  elements.checkButton.disabled = false;
  elements.saveProgressButton.disabled = false;
  elements.resetButton.disabled = false;
  elements.hintButton.disabled = false;
  elements.pythonStatus.textContent = language === "python"
    ? "IDLE-style Python editor · Tab inserts four spaces · colon + Enter indents the next line."
    : (language === "pseudocode" ? "Pseudocode editor" : "Written-answer editor");
  codeIsDirty = false;
  setSaveStatus(currentProgress.has(activity.id) ? "saved" : "idle", currentProgress.has(activity.id) ? "Saved progress loaded" : "No unsaved changes");
  renderRelatedPractice(activity, elements.challengeRelatedPractice);
  showView("challengeView");
  setTimeout(() => elements.codeEditor.focus(), 0);
  window.scrollTo({ top: 0, behavior: "smooth" });
}

function openSddFluencyChallenge(activity) {
  resetOfficialResponseWorkspace();
  activeFluencyChallenge = { activity, state: fluencyProgressState(activity) };
  renderSddFluencyAttempt();
}

async function checkActiveFluencyAttempt() {
  if (!activeFluencyChallenge) return;
  const { activity, state } = activeFluencyChallenge;
  const task = activity.tasks[state.index];
  const response = elements.codeEditor.value;
  if (!String(response).trim()) {
    elements.feedbackBox.textContent = "Enter an answer before checking.";
    elements.feedbackBox.className = "feedback error";
    return;
  }
  const language = inferFluencyLanguage(task);
  if (language === "python") {
    const execution = await runCode(response, currentTask.sampleInputs || []);
    elements.outputBox.textContent = execution.output || "(No output)";
    if (execution.error) {
      showExecutionError(execution.error);
      state.checkCounts[state.index] = Number(state.checkCounts[state.index] || 0) + 1;
      if (state.checkCounts[state.index] === 1) state.firstPassCorrect[state.index] = false;
      state.responses[state.index] = response;
      elements.feedbackBox.textContent = "Python stopped with an error. Repair it, then check this practice again.";
      elements.feedbackBox.className = "feedback error";
      await saveActiveFluencyProgress({ countAttempt: true, correct: false });
      return;
    }
    hideExecutionError();
  }
  state.checkCounts[state.index] = Number(state.checkCounts[state.index] || 0) + 1;
  const correct = await fluencyTaskCorrectAsync(activity, task, response);
  if (state.checkCounts[state.index] === 1) state.firstPassCorrect[state.index] = Boolean(correct);
  if (correct && !state.secondsByPractice[state.index]) {
    state.secondsByPractice[state.index] = Math.max(1, Math.round((Date.now() - Number(state.startedAt || Date.now())) / 1000));
  }
  state.responses[state.index] = response;
  state.correct[state.index] = correct;
  await saveActiveFluencyProgress({ countAttempt: true, correct });
  removeNextQuestionButton(elements.feedbackBox);
  if (!correct) {
    elements.feedbackBox.innerHTML = `<strong>Not yet.</strong> ${escapeHtml(task.explanation || `One acceptable answer is ${task.answer}`).replaceAll("\n", "<br>")}`;
    elements.feedbackBox.className = "feedback error";
    elements.challengeStatus.textContent = "Needs work";
    elements.challengeStatus.className = "status-pill needs-work";
    return;
  }
  elements.feedbackBox.innerHTML = `<strong>Correct.</strong> ${escapeHtml(task.explanation || "That pattern is secure.").replaceAll("\n", "<br>")}`;
  elements.feedbackBox.className = "feedback success";
  elements.challengeStatus.textContent = "Practice complete";
  ...
  nextButton.textContent = state.index >= activity.tasks.length - 1 ? "Finish practice set →" : "Next practice →";
  nextButton.addEventListener("click", async () => {
    nextButton.disabled = true;
    if (state.index >= activity.tasks.length - 1) {
      const summary = fluencyMasterySummary(state);
      await saveActiveFluencyProgress({ complete: true });
      elements.feedbackBox.innerHTML = summary.mastered
        ? `<strong>Mastered.</strong> ${summary.firstPass}/6 first-pass correct · ${summary.hints} hint${summary.hints === 1 ? "" : "s"} · ${Math.max(1, Math.round(summary.seconds / 60))} min active practice.`
        : `<strong>Practice set complete.</strong> ${summary.firstPass}/6 first-pass correct · ${summary.hints} hint${summary.hints === 1 ? "" : "s"}. This skill is marked <strong>Developing</strong> and will return in Daily Mastery.`;
      elements.feedbackBox.className = "feedback success";
      elements.challengeStatus.textContent = summary.mastered ? "Mastered" : "Developing";
      elements.challengeStatus.className = `status-pill ${summary.mastered ? "complete" : "working"}`;
      renderRelatedPractice(activity, elements.challengeRelatedPractice);
    ...
    await saveActiveFluencyProgress();
    renderSddFluencyAttempt();
  });
  actions.appendChild(nextButton);
  elements.feedbackBox.appendChild(actions);
}

function openActivity(activity) {
  if (!isActivityUnlocked(activity)) {
    showMessage("Master the prerequisite skill enough to advance first. Daily Mastery can help you strengthen it.", "error");
    return;
  }
  currentActivity = activity;
  if (activity.type === "fluency-sprint" && activity.areaId === "sdd") {
    openSddFluencyChallenge(activity);
  } else if (["code", "debug", "visualiser"].includes(activity.type)) {
    openTask(activity);
  } else {
...
async function saveGenericProgress(activity, updates = {}, { countAttempt = false, correct = false } = {}) {
  ...
  const mastery = computeMasteryAttemptFields(activity, old, {
    countAttempt,
    correct,
    completed,
    explicitMastered: Object.prototype.hasOwnProperty.call(updates, "mastered") ? updates.mastered : undefined
  });
  const data = {
    ...baseGenericProgressData(activity, old),
    ...updates,
    completed,
    status: mastery.masteryStatus === "repair" ? "needs_work" : completed ? "complete" : (countAttempt ? "needs_work" : "in_progress"),
    attempts: mastery.attempts,
    correctAttempts: Number(old.correctAttempts || 0) + (countAttempt && correct ? 1 : 0),
    ...
    incorrectStreak: mastery.incorrectStreak,
    mastered: mastery.mastered,
    masteryStatus: mastery.masteryStatus,
    masteryVersion: mastery.masteryVersion,
    masteredAt: mastery.masteredAt,
    lastAttemptCorrect: countAttempt ? Boolean(correct) : old.lastAttemptCorrect,
    lastActivityAt: serverTimestamp(),
    lastSavedAt: serverTimestamp()
  };
...
async function loadOrClaimPeerReview() {
  ...
  if (!hasSchoolClass()) { elements.peerMarkingCard.classList.add("hidden"); return; }
  ...
  return `
    <div class="model-answer-panel">
      ...
      <div class="activity-actions peer-choice-actions">
        ...
        ${hasSchoolClass() ? '<button id="requestPeerReviewButton" type="button">Request anonymous peer check</button>' : ''}
...
function renderSqlActivity(activity, progress) {
  elements.activityContent.innerHTML = `
    <div class="sql-task-introduction">
      ...
      <p class="help-text"><strong>Live practice database:</strong> this runs against a genuine browser-based relational SQLite database with real tables, records, keys and referential-integrity checks. A fresh copy is created for each attempt, so INSERT, UPDATE and DELETE cannot damage another activity.</p>
    </div>
  ...
  if (hintButton) {
    hintButton.addEventListener("click", async () => {
      ...
      try { await saveGenericProgress(activity, { sqlDraft: editor.value, hintsUsed: Number(oldProgress.hintsUsed || 0) + 1 }); }
      catch (error) { console.warn("SQL hint use could not be saved:", error); }
    });
  }
...
function normaliseFluencyAnswer(value) {
  return String(value || "")
    .trim()
    .replace(/[“”]/g, '"')
    .replace(/[‘’]/g, "'")
...
function looksLikeSqlStatement(value) {
  return /^(select|insert\s+into|update|delete\s+from)\b/i.test(String(value || "").trim());
}

async function pythonStructuralSignature(code) {
  const python = await ensurePyodide();
  python.globals.set("signature_code", String(code || ""));
  const raw = await python.runPythonAsync(`
import ast, json
try:
    tree = ast.parse(signature_code)
    counts = {}
    calls = {}
    compares = {}
    boolops = {}
    ...
    for node in ast.walk(tree):
        ...
        if name in {"Assign","If","For","While","List","Subscript","AugAssign"}:
        ...
        if isinstance(node, ast.Compare):
            for op in node.ops:
                ...
                compares[opname] = compares.get(opname, 0) + 1
        if isinstance(node, ast.BoolOp):
            opname = type(node.op).__name__
            boolops[opname] = boolops.get(opname, 0) + 1
        if isinstance(node, ast.BinOp):
            opname = type(node.op).__name__
            binops[opname] = binops.get(opname, 0) + 1
        ...
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Name) and node.func.id == "print":
            for arg in node.args:
                if isinstance(arg, ast.Constant) and isinstance(arg.value, str):
                    print_strings[arg.value] = print_strings.get(arg.value, 0) + 1
    result = {"ok": True, "counts": counts, "calls": calls, "compares": compares, "boolops": boolops, "binops": binops, "numbers": numbers, "printStrings": print_strings}
...
function signatureContains(actual = {}, expected = {}) {
  for (const group of ["counts", "calls", "compares", "boolops", "binops", "numbers", "printStrings"]) {
    for (const [key, amount] of Object.entries(expected[group] || {})) {
      if (Number(actual[group]?.[key] || 0) < Number(amount || 0)) return false;
    }
  }
  return true;
}

async function pythonFluencyEquivalent(task, response) {
  const accepted = Array.isArray(task.answer) ? task.answer : [task.answer];
  const model = accepted.find(answer => String(answer || "").trim()) || "";
  const [studentSignature, modelSignature] = await Promise.all([
    pythonStructuralSignature(response),
    pythonStructuralSignature(model)
  ]);
  if (!studentSignature.ok || !modelSignature.ok || !signatureContains(studentSignature, modelSignature)) return false;
  if ((task.patterns || []).length && !task.patterns.every(pattern => {
    try { return new RegExp(pattern, "i").test(String(response)); }
    catch { return String(response).toLowerCase().includes(String(pattern).toLowerCase()); }
  })) return false;

  const inputs = inferFluencySampleInputs(model);
  try {
    const [studentRun, modelRun] = await Promise.all([runCode(response, inputs), runCode(model, inputs)]);
    if (!studentRun.error && !modelRun.error) {
      return String(studentRun.output || "").trim() === String(modelRun.output || "").trim();
    }
  } catch (error) {
    console.warn("Fluency execution comparison fell back to structure:", error);
  }
  // Some short National 5 fragments intentionally depend on variables supplied
  // by the surrounding question. In those cases AST structure is the fair check.
  return true;
}

function sqlIdentifiers(sql) {
  const keywords = new Set(["select","from","where","and","or","not","order","by","asc","desc","insert","into","values","update","set","delete","as","on","join","inner","left","right","null","is","like","true","false"]);
  return [...new Set((String(sql || "").match(/\b[A-Za-z_][A-Za-z0-9_]*\b/g) || []).filter(token => !keywords.has(token.toLowerCase())))];
}

function sqlTableNames(sql) {
  const names = [];
  for (const match of String(sql || "").matchAll(/\b(?:from|into|update|join)\s+\[?([A-Za-z_][A-Za-z0-9_]*)\]?/gi)) names.push(match[1]);
  for (const match of String(sql || "").matchAll(/\bdelete\s+from\s+\[?([A-Za-z_][A-Za-z0-9_]*)\]?/gi)) names.push(match[1]);
  return [...new Set(names)];
}

async function sqlFluencyEquivalent(task, response) {
  const accepted = Array.isArray(task.answer) ? task.answer : [task.answer];
  const model = accepted.find(answer => looksLikeSqlStatement(answer));
  if (!model || !looksLikeSqlStatement(response)) return false;
  const SQL = await ensureSqlJsModule();
  const database = new SQL.Database();
  try {
    const tables = [...new Set([...sqlTableNames(model), ...sqlTableNames(response)])];
    if (!tables.length) return false;
    const identifiers = [...new Set([...sqlIdentifiers(model), ...sqlIdentifiers(response)])]
      .filter(name => !tables.some(table => table.toLowerCase() === name.toLowerCase()));
    const columns = identifiers.length ? identifiers : ["id", "name", "value"];
    for (const table of tables) {
      const safeTable = table.replace(/]/g, "]]" );
      const defs = columns.map(column => `[${column.replace(/]/g, "]]" )}]`).join(", ");
      database.exec(`CREATE TABLE [${safeTable}] (${defs});`);
      const values1 = columns.map((column, index) => {
        const eq = new RegExp(`\\b${column}\\s*=\\s*(?:[\"']([^\"']+)[\"']|([0-9]+))`, "i").exec(`${model} ${response}`);
        if (eq?.[1] !== undefined) return `'${String(eq[1]).replace(/'/g, "''")}'`;
        if (eq?.[2] !== undefined) return String(Number(eq[2]));
        return index % 2 ? `'Aberdeen'` : String(20 + index);
      });
      const values2 = columns.map((_, index) => index % 2 ? `'Glasgow'` : String(5 + index));
      database.exec(`INSERT INTO [${safeTable}] VALUES (${values1.join(",")});`);
      database.exec(`INSERT INTO [${safeTable}] VALUES (${values2.join(",")});`);
    }
    const runOnFreshCopy = sql => {
      const bytes = database.export();
      const copy = new SQL.Database(bytes);
      try {
        const results = copy.exec(normaliseNational5Sql(sql));
        const finalTables = {};
        for (const table of tables) finalTables[table] = lastSqlResult(copy.exec(`SELECT * FROM [${table}] ORDER BY rowid;`));
        return { result: lastSqlResult(results), tables: finalTables };
      } finally { copy.close(); }
    };
    const student = runOnFreshCopy(response);
    const expected = runOnFreshCopy(model);
    return JSON.stringify(student) === JSON.stringify(expected);
  } catch (error) {
    console.warn("SQL fluency engine comparison fell back to normalised SQL:", error);
    const loose = value => normaliseNational5Sql(value).replace(/;+$|\s+/g, match => match.includes(";") ? "" : " ").trim().toLowerCase().replace(/["']/g, "'");
    return accepted.some(answer => loose(answer) === loose(response));
  } finally {
    database.close();
  }
}

async function fluencyTaskCorrectAsync(activity, task, response) {
  if (task.format === "choice") return Number(response) === Number(task.answerIndex);
  const expected = Array.isArray(task.answer) ? task.answer.join("\n") : String(task.answer || "");
  if (activity?.areaId === "ddd" && (looksLikeSqlStatement(response) || looksLikeSqlStatement(expected))) {
    return sqlFluencyEquivalent(task, response);
  }
  if (task.format === "code" && inferFluencyLanguage(task) === "python") {
    return pythonFluencyEquivalent(task, response);
  }
  return fluencyTaskCorrect(task, response);
}

function fluencyTaskCorrect(task, response) {
  const actual = normaliseFluencyAnswer(response);
  if (task.format === "choice") return Number(response) === Number(task.answerIndex);
  const accepted = Array.isArray(task.answer) ? task.answer : [task.answer];
  if (accepted.some(answer => normaliseFluencyAnswer(answer) === actual)) return true;
  if (task.patterns?.length) {
    return task.patterns.every(pattern => {
      try { return new RegExp(pattern, "i").test(String(response)); }
      catch { return String(response).toLowerCase().includes(String(pattern).toLowerCase()); }
    });
  }
  // Text responses allow punctuation and explanatory words around the accepted phrase.
  return task.format === "text" && accepted.some(answer => {
    const target = normaliseFluencyAnswer(answer);
    return target.length <= 35 && actual.includes(target);
  });
}

function renderFluencySprint(activity, progress) {
  const saved = progress.fluency || {};
  let state = {
    index: Math.min(Number(saved.index || 0), 5),
    responses: Array.isArray(saved.responses) ? [...saved.responses] : Array(6).fill(""),
    correct: Array.isArray(saved.correct) ? [...saved.correct] : Array(6).fill(false),
    checkCounts: Array.isArray(saved.checkCounts) ? [...saved.checkCounts] : Array(6).fill(0),
    firstPassCorrect: Array.isArray(saved.firstPassCorrect) ? [...saved.firstPassCorrect] : Array(6).fill(null),
    hintsByPractice: Array.isArray(saved.hintsByPractice) ? [...saved.hintsByPractice] : Array(6).fill(0),
    secondsByPractice: Array.isArray(saved.secondsByPractice) ? [...saved.secondsByPractice] : Array(6).fill(0),
    startedAt: Date.now()
  };
  const tasks = activity.tasks || [];
  elements.activityContent.innerHTML = `<p>This activity gives six deliberate practices of the same skill. Support fades as you become more independent.</p><div class="fluency-progress-strip">${tasks.map((_, index) => `<span data-fluency-dot="${index}">${index + 1}</span>`).join("")}</div>`;
  elements.activityInteraction.innerHTML = `<section class="fluency-shell"><div id="fluencyTaskMeta" class="activity-meta"></div><h3 id="fluencyTaskPrompt"></h3><div id="fluencyTaskInput"></div><div id="fluencyHint" class="hint-box hidden"></div><div class="activity-actions"><button id="checkFluencyButton" type="button">Check practice</button><button id="fluencyHintButton" type="button" class="secondary">Show support</button><button id="nextFluencyButton" type="button" class="secondary hidden">Next practice</button></div><div id="fluencyFeedback" class="feedback hidden" aria-live="polite"></div></section>`;
  const meta = elements.activityInteraction.querySelector('#fluencyTaskMeta');
  const prompt = elements.activityInteraction.querySelector('#fluencyTaskPrompt');
  const inputWrap = elements.activityInteraction.querySelector('#fluencyTaskInput');
  const hint = elements.activityInteraction.querySelector('#fluencyHint');
  const feedback = elements.activityInteraction.querySelector('#fluencyFeedback');
  const checkButton = elements.activityInteraction.querySelector('#checkFluencyButton');
  const hintButton = elements.activityInteraction.querySelector('#fluencyHintButton');
  const nextButton = elements.activityInteraction.querySelector('#nextFluencyButton');

  const saveState = async (completed = false, countAttempt = false, correct = false) => {
    const summary = fluencyMasterySummary(state);
    await saveGenericProgress(activity, {
      fluency: state,
      completed,
      mastered: completed ? summary.mastered : currentProgress.get(activity.id)?.mastered,
      masteryStatus: completed ? (summary.mastered ? "mastered" : "developing") : currentProgress.get(activity.id)?.masteryStatus,
      fluencyFirstPassCorrect: summary.firstPass,
      fluencyHintsUsed: summary.hints,
      fluencySeconds: summary.seconds
    }, { countAttempt, correct });
  };
  const render = () => {
    const task = tasks[state.index];
    const support = state.index <= 1 ? 'Worked pattern available' : state.index <= 3 ? 'Compact reminder' : state.index === 4 ? 'Generate from the prompt' : 'Near transfer';
    meta.innerHTML = `<span class="activity-chip">Practice ${state.index + 1} of 6</span><span class="activity-chip">${support}</span>`;
    prompt.textContent = task.prompt;
    hint.textContent = task.hint || (state.index <= 1 ? `Pattern: ${task.answer}` : state.index <= 3 ? 'Use the same structure as the previous successful attempt.' : 'Break the requirement into input, process and output or identify the exact syntax pattern needed.');
    hint.classList.add('hidden');
    feedback.className = 'feedback hidden';
    feedback.textContent = '';
    nextButton.classList.add('hidden');
    checkButton.disabled = false;
    document.querySelectorAll('[data-fluency-dot]').forEach(dot => {
      const i = Number(dot.dataset.fluencyDot);
      dot.classList.toggle('complete', Boolean(state.correct[i]));
      dot.classList.toggle('current', i === state.index);
    });
    if (task.format === 'choice') {
      inputWrap.innerHTML = `<div class="option-list">${task.options.map((option, index) => `<label class="option-label"><input type="radio" name="fluency-answer" value="${index}" ${String(state.responses[state.index]) === String(index) ? 'checked' : ''}> <span>${escapeHtml(option)}</span></label>`).join('')}</div>`;
    } else if (task.format === 'code') {
      inputWrap.innerHTML = `<label>Your code or pseudocode<textarea id="fluencyResponse" class="code-editor fluency-editor" spellcheck="false">${escapeHtml(state.responses[state.index] || task.starter || '')}</textarea></label>`;
      const responseEditor = inputWrap.querySelector('#fluencyResponse');
      enhanceCodeTextarea(responseEditor, inferFluencyLanguage(task));
      setTimeout(() => responseEditor?.focus(), 0);
    } else {
      inputWrap.innerHTML = `<label>Your answer<textarea id="fluencyResponse" class="written-response compact-response">${escapeHtml(state.responses[state.index] || '')}</textarea></label>`;
      setTimeout(() => inputWrap.querySelector('#fluencyResponse')?.focus(), 0);
    }
  };
  const readResponse = () => {
    const task = tasks[state.index];
    if (task.format === 'choice') return inputWrap.querySelector('input[name="fluency-answer"]:checked')?.value ?? '';
    return inputWrap.querySelector('#fluencyResponse')?.value || '';
  };
  checkButton.addEventListener('click', async () => {
    const task = tasks[state.index];
    ...
    const correct = await fluencyTaskCorrectAsync(activity, task, response);
    if (state.checkCounts[state.index] === 1) state.firstPassCorrect[state.index] = Boolean(correct);
    if (correct && !state.secondsByPractice[state.index]) state.secondsByPractice[state.index] = Math.max(1, Math.round((Date.now() - Number(state.startedAt || Date.now())) / 1000));
    state.correct[state.index] = correct;
    await saveState(false, true, correct);
    feedback.innerHTML = correct
      ? `<strong>Correct.</strong> ${escapeHtml(task.explanation || 'That pattern is secure.')}`.replaceAll('\n','<br>')
      : `<strong>Not yet.</strong> ${escapeHtml(task.explanation || `One acceptable answer is ${task.answer}`)}`.replaceAll('\n','<br>');
    feedback.className = `feedback ${correct ? 'success' : 'error'}`;
    if (correct) {
      checkButton.disabled = true;
      nextButton.classList.remove('hidden');
      nextButton.textContent = state.index === tasks.length - 1 ? 'Finish practice set' : 'Next practice →';
    } else {
  ...
  hintButton.addEventListener('click', async () => {
    ...
    if (wasHidden) {
      state.hintsByPractice[state.index] = Number(state.hintsByPractice[state.index] || 0) + 1;
      const oldProgress = currentProgress.get(activity.id) || {};
      await saveGenericProgress(activity, { fluency: state, hintsUsed: Number(oldProgress.hintsUsed || 0) + 1 });
    }
  });
  nextButton.addEventListener('click', async () => {
    if (!state.correct[state.index]) return;
    if (state.index >= tasks.length - 1) {
      const summary = fluencyMasterySummary(state);
      await saveState(true);
      setGenericFeedback(summary.mastered
        ? `Mastered — ${summary.firstPass}/6 first-pass correct, ${summary.hints} hint${summary.hints === 1 ? '' : 's'}.`
        : `Practice set complete — ${summary.firstPass}/6 first-pass correct, ${summary.hints} hint${summary.hints === 1 ? '' : 's'}. This skill remains Developing and will return in Daily Mastery.`, summary.mastered ? 'success' : 'info');
      setGenericStatus(activity);
      renderRelatedPractice(activity, elements.activityRelatedPractice);
      return;
    }
...
function renderGenericActivity(activity) {
  ...
  if (activity.type === "fluency-sprint") {
    renderFluencySprint(activity, progress);
  } else if (activity.type === "erd-builder") {
    ...
    const renderModelPanel = () => {
      ...
      modelPanel.querySelector("#completeOfficialReviewButton").addEventListener("click", async () => {
        ...
        setGenericFeedback(hasSchoolClass() ? "Your self-review has been saved. You can now request an anonymous peer check or complete without one." : "Your self-review has been saved. Complete the task when you are satisfied with your answer.", "success");
        setGenericStatus(activity);
      });
      const requestPeerButton = modelPanel.querySelector("#requestPeerReviewButton");
      requestPeerButton?.addEventListener("click", async () => {
        try {
          await requestAnonymousPeerReview(activity, Number(selfMarkInput.value), reflectionInput.value);
          ...
          requestPeerButton.disabled = true;
          requestPeerButton.textContent = "Peer check requested";
        } catch (error) {
      ...
      if (requestPeerButton && (currentProgress.get(activity.id) || {}).peerReviewId) {
        requestPeerButton.disabled = true;
        requestPeerButton.textContent = "Peer check requested";
      }
...
function openTask(task) {
  ...
  enhanceCodeTextarea(elements.codeEditor, "python");
  refreshCodeHighlight(elements.codeEditor);
...
elements.hintButton.addEventListener("click", async () => {
  ...
  try {
    if (activeFluencyChallenge) {
      const old = currentProgress.get(activeFluencyChallenge.activity.id) || {};
      const state = activeFluencyChallenge.state;
      state.hintsByPractice[state.index] = Number(state.hintsByPractice[state.index] || 0) + 1;
      await saveGenericProgress(activeFluencyChallenge.activity, {
        fluency: state,
...
function progressDocumentId(taskId) {
  return `${learningScopeId()}_${auth.currentUser.uid}_${taskId}`;
}
...
async function saveProgressDraft({ silent = false, automatic = false } = {}) {
  if (activeFluencyChallenge) {
    await saveActiveFluencyProgress();
    if (!silent) {
      elements.feedbackBox.textContent = "Fluency progress saved. This does not count as an attempt.";
      elements.feedbackBox.className = "feedback success";
    }
    return;
  }
...
elements.checkButton.addEventListener("click", async () => {
  ...
  if (activeFluencyChallenge) {
    setRunningButtons(true);
    try { await checkActiveFluencyAttempt(); }
    catch (error) {
      console.error("Fluency check failed:", error);
      elements.feedbackBox.textContent = error.message || "The attempt could not be checked.";
      elements.feedbackBox.className = "feedback error";
    } finally {
      const language = inferFluencyLanguage(activeFluencyChallenge.activity.tasks[activeFluencyChallenge.state.index]);
      elements.runButton.disabled = language !== "python";
      elements.visualiseButton.disabled = language !== "python";
      elements.checkButton.disabled = false;
    }
...
async function saveAttempt(correct, executionError = null, gradingFeedback = []) {
  const progressRef = doc(db, "progress", progressDocumentId(currentTask.id));
  const old = currentProgress.get(currentTask.id) || {};
  const activity = currentActivity || allActivities.find(item => item.id === currentTask.id) || currentTask;
  const mastery = computeMasteryAttemptFields(activity, old, { countAttempt: true, correct, completed: Boolean(old.completed || correct) });
  ...
  const data = {
    ...
    status: mastery.masteryStatus === "repair" ? "needs_work" : Boolean(old.completed || correct) ? "complete" : "needs_work",
    attempts: mastery.attempts,
    correctAttempts: Number(old.correctAttempts || 0) + (correct ? 1 : 0),
    ...
    masteryVersion: mastery.masteryVersion,
    masteredAt: mastery.masteredAt,
    lastAttemptCorrect: Boolean(correct),
    lastCode: elements.codeEditor.value,
...
async function recordVisualiserUse() {
  if (activeFluencyChallenge) {
    const old = currentProgress.get(activeFluencyChallenge.activity.id) || {};
    activeFluencyChallenge.state.responses[activeFluencyChallenge.state.index] = elements.codeEditor.value;
    await saveGenericProgress(activeFluencyChallenge.activity, {
      fluency: activeFluencyChallenge.state,
      lastCode: elements.codeEditor.value,
      visualiserUses: (old.visualiserUses || 0) + 1
    });
    codeIsDirty = false;
    clearScheduledAutoSave();
    setSaveStatus("saved", "Visualiser use saved");
    return;
  }
  if (!currentTask || !auth.currentUser || currentProfile?.role !== "student") return;
  const progressRef = doc(db, "progress", progressDocumentId(currentTask.id));
...
async function loadPupilGameAttempts() {
  ...
  try {
    ...
    console.warn("Computing Arcade history could not be loaded:", error);
    currentGameAttempts = [];
  }
}
...
const ARCADE_GAME_CONFIG = Object.freeze({
  "code-flyer": {
    title: "Code Flyer",
    icon: "🐦",
    areaIds: ["sdd"],
    skillIds: ["predict-output", "trace-code", "variables", "assignment", "selection", "iteration", "operators", "input"],
    tagline: "Fly through the correct output gate.",
    theme: "flyer"
  },
  "trace-race": {
    title: "Trace Race",
    icon: "🏁",
    areaIds: ["sdd"],
    skillIds: ["trace-code", "predict-output", "variables", "selection", "fixed-loops", "conditional-loops", "arrays"],
    tagline: "Keep your runner moving by tracing code correctly.",
    theme: "race"
  },
  "binary-bomb": {
    title: "Binary Bomb",
    icon: "💣",
    areaIds: ["cs"],
    skillIds: ["binary", "denary", "conversion", "place-values", "bits", "floating-point", "mantissa", "exponent"],
    tagline: "Cut the correct wire before the timer reaches zero.",
    theme: "bomb"
  },
  "sql-sprint": {
    title: "SQL Sprint",
    icon: "🗃️",
    areaIds: ["ddd"],
    skillIds: ["sql", "select", "where", "order-by", "insert", "update", "delete", "query"],
    tagline: "Choose the query, clause or result that gets you to the finish.",
    theme: "sql"
  },
  "cyber-escape": {
    title: "Cyber Escape",
    icon: "🔐",
    areaIds: ["cs"],
    skillIds: ["security", "firewall", "encryption", "electronic-communications", "network-traffic"],
    tagline: "Unlock each door by making the secure choice.",
    theme: "cyber"
  }
});

function arcadeIsOpenForCurrentPupil() {
  if (isIndependentLearner()) return true;
  if (!hasSchoolClass()) return true;
  return currentClassSettings.arcadeOpen === true;
}

function requiredActivitiesInUnit(unitId) {
  return requiredActivities().filter(activity => activity.unitId === unitId);
}

function arcadeActivityReached(activity) {
  if (!activity) return false;
  const progress = currentProgress.get(activity.id);
  return Boolean(
...
function arcadeUnlockedSkillIds(areaIds = null) {
  const allowedAreas = areaIds ? new Set(areaIds) : null;
  const skills = new Set();
  visibleActivities().forEach(activity => {
    if (allowedAreas && !allowedAreas.has(activity.areaId)) return;
    if (!arcadeActivityReached(activity)) return;
    activitySkillIds(activity).forEach(skillId => skills.add(skillId));
  });
  return skills;
}

function arcadeQuestionAreaId(question) {
  return question.areaId || unitById(question.unitId)?.areaId || "sdd";
}

function questionMatchesArcadeGame(question, config) {
  if (!config.areaIds.includes(arcadeQuestionAreaId(question))) return false;
  const questionSkills = Array.isArray(question.skills) ? question.skills : [];
  if (!config.skillIds.length) return true;
  return questionSkills.some(skillId => config.skillIds.includes(skillId));
}

function unlockedArcadeQuestion(question, config) {
  if (!questionMatchesArcadeGame(question, config)) return false;
  const unlockedSkills = arcadeUnlockedSkillIds(config.areaIds);
  const questionSkills = Array.isArray(question.skills) ? question.skills : [];
  if (questionSkills.length) return questionSkills.some(skillId => unlockedSkills.has(skillId));
  const unitActivities = requiredActivitiesInUnit(question.unitId);
  return unitActivities.some(arcadeActivityReached);
}

function nextArcadeActivity(areaIds = null) {
  const allowedAreas = areaIds ? new Set(areaIds) : null;
  return requiredActivities().find(activity => {
    if (allowedAreas && !allowedAreas.has(activity.areaId)) return false;
    if (!isActivityUnlocked(activity)) return false;
    return !isActivityMastered(activity, currentProgress.get(activity.id));
  }) || null;
}

function nextArcadeQuestion(question, config) {
  if (!questionMatchesArcadeGame(question, config)) return false;
  const next = nextArcadeActivity(config.areaIds);
  if (!next) return false;
  const questionSkills = Array.isArray(question.skills) ? question.skills : [];
  const nextSkills = new Set(activitySkillIds(next));
  if (question.unitId === next.unitId && !questionSkills.length) return true;
  return question.unitId === next.unitId && questionSkills.some(skillId => nextSkills.has(skillId));
}

function quickArcadeQuestionPool(gameId) {
  const config = ARCADE_GAME_CONFIG[gameId];
  if (!config) return [];
  let pool = SPACED_QUESTIONS.filter(question => arcadePracticeMode === "next"
    ? nextArcadeQuestion(question, config)
    : unlockedArcadeQuestion(question, config));

  return pool;
}

function gameUnlockState(game) {
  const unitId = game === "debug-rescue" ? "sdd-errors-17" : "sdd-logic-07";
  const unit = unitById(unitId);
  const unitActivities = requiredActivitiesInUnit(unitId);
  const reached = unitActivities.some(arcadeActivityReached);
  const next = nextArcadeActivity(["sdd"]);
  const nextPractice = arcadePracticeMode === "next" && next?.unitId === unitId;
  return {
    unlocked: arcadePracticeMode === "unlocked" ? reached : nextPractice,
    previewOnly: arcadePracticeMode === "next",
    unitId,
    unitTitle: unit?.title || "the related unit"
  };
}
...
function allArcadeStats() {
  const rows = currentGameAttempts;
  const correct = rows.filter(item => item.correct).length;
  return {
    attempts: rows.length,
    correct,
...
function renderGamesLabCard() {
  const stats = allArcadeStats();
  const open = arcadeIsOpenForCurrentPupil();
  elements.gamesLabCard?.classList.toggle("arcade-closed-card", !open);
  elements.openGamesLabButton.disabled = !open;
  elements.openGamesLabButton.textContent = open ? "Open Computing Arcade" : "Arcade closed";
  if (!open) {
    elements.gamesLabDescription.textContent = "Your teacher has the Computing Arcade closed for this class just now.";
    elements.gamesLabMeta.innerHTML = `<span class="activity-chip">Teacher controlled</span><span class="activity-chip">Your progress is safe</span>`;
    return;
  }
  elements.gamesLabDescription.textContent = stats.attempts
    ? `${stats.correct}/${stats.attempts} successful Arcade responses. Games use skills you have reached in the course.`
    : "Play short Computing games using skills you have reached in Python, databases and computer systems.";
  elements.gamesLabMeta.innerHTML = `<span class="activity-chip">${stats.attempts} saved responses</span><span class="activity-chip">${stats.attempts ? `${stats.accuracy}% accuracy` : "Ready to play"}</span><span class="activity-chip">Linked to mastery</span>`;
}

function renderArcadePracticeScope() {
  if (!elements.arcadeUnlockedModeButton) return;
  elements.arcadeUnlockedModeButton.classList.toggle("active", arcadePracticeMode === "unlocked");
  elements.arcadeNextModeButton.classList.toggle("active", arcadePracticeMode === "next");
  const nextRows = AREAS.map(area => ({ area, activity: nextArcadeActivity([area.id]) })).filter(row => row.activity);
  if (nextRows.length) {
    elements.arcadeNextSkillSummary.innerHTML = `<strong>Next-skill practice follows each working level independently:</strong> ${nextRows.map(({ area, activity }) => `${escapeHtml(area.shortTitle)} — ${escapeHtml(activity.title)}`).join(" · ")}`;
    elements.arcadeNextModeButton.disabled = false;
  } else {
    elements.arcadeNextSkillSummary.innerHTML = `<strong>Next skill practice:</strong> no unlocked unmastered step is waiting right now.`;
    elements.arcadeNextModeButton.disabled = true;
    if (arcadePracticeMode === "next") arcadePracticeMode = "unlocked";
  }
}

function renderQuickArcadeCards() {
  Object.entries(ARCADE_GAME_CONFIG).forEach(([gameId]) => {
    const button = elements[`arcade-${gameId}-button`];
    const text = elements[`arcade-${gameId}-text`];
    if (!button || !text) return;
    const pool = quickArcadeQuestionPool(gameId);
    const available = pool.length > 0;
    button.disabled = !available;
    button.textContent = available
      ? (arcadePracticeMode === "next" ? "Practise next skill" : "Play")
      : "Not ready yet";
    text.textContent = available
      ? (arcadePracticeMode === "next"
          ? `${pool.length} practice question${pool.length === 1 ? "" : "s"} from your next unmastered step. Practice-only: this will not unlock the pathway.`
          : `${pool.length} question${pool.length === 1 ? "" : "s"} available from skills you have reached.`)
      : (arcadePracticeMode === "next" ? "This game does not match your next skill yet." : "Reach the related course skills to unlock this cabinet.");
  });
}

function renderGamesLabMenu() {
  if (!arcadeIsOpenForCurrentPupil()) {
    showView("pupilView");
    renderGamesLabCard();
    showMessage("The Computing Arcade is closed for your class. Your teacher can open it from the class dashboard.", "info");
    return;
  }
  renderArcadePracticeScope();
  ...
  const allStats = allArcadeStats();
  elements.gamesLabSummary.innerHTML = [
    statCard(allStats.attempts ? `${allStats.accuracy}%` : "—", "Arcade accuracy"),
  ...
  const describeCabinet = (state) => {
    if (state.unlocked && state.previewOnly) return `Available as next-skill practice in ${state.unitTitle}. Results are practice-only and do not advance mastery.`;
    if (state.unlocked) return `Unlocked because you have reached ${state.unitTitle}. Results feed your review profile.`;
    return arcadePracticeMode === "next"
      ? `Not your next skill. Your next-skill practice stays focused on one step at a time.`
      : `Reach ${state.unitTitle} in the pathway to unlock this cabinet.`;
  };
  elements.debugGameUnlockText.textContent = describeCabinet(debugUnlock);
  elements.logicGameUnlockText.textContent = describeCabinet(logicUnlock);
  elements.openDebugGameButton.disabled = !debugUnlock.unlocked;
  elements.openLogicGameButton.disabled = !logicUnlock.unlocked;
  elements.openDebugGameButton.textContent = debugUnlock.unlocked ? (debugUnlock.previewOnly ? "Practise next skill" : "Play Debug Rescue") : "Not ready yet";
  elements.openLogicGameButton.textContent = logicUnlock.unlocked ? (logicUnlock.previewOnly ? "Practise next skill" : "Play Logic Showdown") : "Not ready yet";
  renderQuickArcadeCards();
}

function setArcadePracticeMode(mode) {
  if (!['unlocked', 'next'].includes(mode)) return;
  arcadePracticeMode = mode;
  renderGamesLabMenu();
}

function arcadeQuestionMarkup(question) {
  if (question.diagramHtml) return `<div class="spaced-diagram arcade-question-media">${question.diagramHtml}</div>`;
  if (question.codeSnippet) return `<pre class="lesson-code arcade-code">${escapeHtml(String(question.codeSnippet).replace(/\\n/g, "\n"))}</pre>`;
  return "";
}

function startQuickArcadeGame(gameId) {
  if (!arcadeIsOpenForCurrentPupil()) return;
  const config = ARCADE_GAME_CONFIG[gameId];
  const pool = quickArcadeQuestionPool(gameId);
  if (!config || !pool.length) {
    showMessage("That Arcade cabinet is not ready for your current skills yet.", "info");
    return;
  }
  quickArcadeState = {
    gameId,
    config,
    previewOnly: arcadePracticeMode === "next",
    queue: shuffled(pool).slice(0, Math.min(8, pool.length)),
    index: 0,
    correct: 0,
    score: 0,
    streak: 0,
  ...
  elements.quickArcadeTitle.textContent = config.title;
  elements.quickArcadeEyebrow.textContent = `${config.icon} Computing Arcade`;
  elements.quickArcadeSubtitle.textContent = config.tagline;
  elements.quickArcadeShell.dataset.arcadeTheme = config.theme;
  elements.quickArcadeSummary.classList.add("hidden");
  elements.quickArcadeQuestionPanel.classList.remove("hidden");
  showView("quickArcadeView");
  renderQuickArcadeQuestion();
}

function renderQuickArcadeQuestion() {
  if (!quickArcadeState || quickArcadeState.index >= quickArcadeState.queue.length) return finishQuickArcadeGame();
  const question = quickArcadeState.queue[quickArcadeState.index];
  quickArcadeState.current = question;
  quickArcadeState.selectedIndex = null;
  quickArcadeState.answered = false;
  quickArcadeState.questionStartedAt = Date.now();
  elements.quickArcadeProgress.textContent = `${quickArcadeState.index + 1}/${quickArcadeState.queue.length}`;
  elements.quickArcadeScore.textContent = `${quickArcadeState.score} pts`;
  elements.quickArcadeStreak.textContent = quickArcadeState.streak ? `🔥 ${quickArcadeState.streak}` : "Streak 0";
  elements.quickArcadeTrackFill.style.width = `${Math.round((quickArcadeState.index / quickArcadeState.queue.length) * 100)}%`;
  elements.quickArcadeQuestionMedia.innerHTML = arcadeQuestionMarkup(question);
  elements.quickArcadeQuestionMedia.classList.toggle("hidden", !elements.quickArcadeQuestionMedia.innerHTML);
  elements.quickArcadePrompt.textContent = question.prompt || "Choose the correct answer.";
  elements.quickArcadeOptions.innerHTML = (question.options || []).map((option, index) => `<button type="button" class="arcade-answer" data-arcade-answer="${index}"><span class="arcade-answer-key">${String.fromCharCode(65 + index)}</span><span>${escapeHtml(option)}</span></button>`).join("");
  elements.quickArcadeOptions.querySelectorAll("[data-arcade-answer]").forEach(button => {
    button.addEventListener("click", () => {
      if (quickArcadeState?.answered) return;
      elements.quickArcadeOptions.querySelectorAll(".selected").forEach(item => item.classList.remove("selected"));
      button.classList.add("selected");
      quickArcadeState.selectedIndex = Number(button.dataset.arcadeAnswer);
    });
  });
  elements.quickArcadeFeedback.className = "feedback hidden";
  elements.quickArcadeFeedback.textContent = "";
  elements.submitQuickArcadeAnswerButton.disabled = false;
  elements.nextQuickArcadeQuestionButton.classList.add("hidden");
  elements.quickArcadePracticeBadge.textContent = quickArcadeState.previewOnly ? "Next skill · practice only" : "Unlocked skills";
}

async function submitQuickArcadeAnswer() {
  if (!quickArcadeState?.current || quickArcadeState.answered) return;
  if (quickArcadeState.selectedIndex === null) {
    elements.quickArcadeFeedback.textContent = "Choose an answer first.";
    elements.quickArcadeFeedback.className = "feedback error";
    return;
  }
  const question = quickArcadeState.current;
  const correct = Number(quickArcadeState.selectedIndex) === Number(question.answer);
  quickArcadeState.answered = true;
  if (correct) {
    quickArcadeState.correct += 1;
    quickArcadeState.streak += 1;
    quickArcadeState.bestStreak = Math.max(quickArcadeState.bestStreak, quickArcadeState.streak);
    quickArcadeState.score += 100 + Math.min(quickArcadeState.streak - 1, 5) * 20;
  } else {
    quickArcadeState.streak = 0;
  }
  const responseTimeSeconds = Math.max(1, Math.round((Date.now() - quickArcadeState.questionStartedAt) / 1000));
  elements.quickArcadeOptions.querySelectorAll("[data-arcade-answer]").forEach(button => {
    const index = Number(button.dataset.arcadeAnswer);
    button.disabled = true;
    button.classList.toggle("correct", index === Number(question.answer));
    button.classList.toggle("incorrect", index === quickArcadeState.selectedIndex && !correct);
  });
  elements.quickArcadeScore.textContent = `${quickArcadeState.score} pts`;
  elements.quickArcadeStreak.textContent = quickArcadeState.streak ? `🔥 ${quickArcadeState.streak}` : "Streak 0";
  elements.quickArcadeFeedback.innerHTML = `<strong>${correct ? "Correct!" : "Not quite."}</strong> ${escapeHtml(question.explanation || "Review the skill and try another one.")}`;
  elements.quickArcadeFeedback.className = `feedback ${correct ? "success" : "error"}`;
  elements.submitQuickArcadeAnswerButton.disabled = true;
  elements.nextQuickArcadeQuestionButton.classList.remove("hidden");
  try {
    await saveGameAttempt({
      game: quickArcadeState.gameId,
      mode: arcadePracticeMode,
      level: "Adaptive",
      challengeId: question.id || `${quickArcadeState.gameId}-${quickArcadeState.index}`,
      correct,
      points: correct ? 1 : 0,
      sessionScore: quickArcadeState.score,
      hintsUsed: 0,
      responseTimeSeconds,
      skillIds: question.skills || [],
      previewOnly: quickArcadeState.previewOnly,
      details: { unitId: question.unitId || "", arcadePracticeMode }
    });
  } catch (error) {
    console.warn("Arcade result could not be saved:", error);
  }
}

function nextQuickArcadeQuestion() {
  if (!quickArcadeState) return;
  quickArcadeState.index += 1;
  renderQuickArcadeQuestion();
}

function finishQuickArcadeGame() {
  if (!quickArcadeState) return;
  const percentage = quickArcadeState.queue.length ? Math.round((quickArcadeState.correct / quickArcadeState.queue.length) * 100) : 0;
  elements.quickArcadeTrackFill.style.width = "100%";
  elements.quickArcadeQuestionPanel.classList.add("hidden");
  elements.quickArcadeSummary.classList.remove("hidden");
  elements.quickArcadeSummary.innerHTML = `<div class="game-result-hero"><span>${quickArcadeState.config.icon}</span><div><h3>${quickArcadeState.config.title} complete</h3><p>${quickArcadeState.correct}/${quickArcadeState.queue.length} correct · ${quickArcadeState.score} points · best streak ${quickArcadeState.bestStreak}</p></div></div>
    <p>${quickArcadeState.previewOnly ? "This was next-skill practice, so it has not been used to unlock the course pathway." : "Saved answers feed your personalised review evidence for skills you have already reached."}</p>
    <div class="activity-actions"><button id="quickArcadePlayAgainButton" type="button">Play again</button><button id="quickArcadeReturnButton" type="button" class="secondary">Return to Arcade</button></div>`;
  elements.quickArcadeSummary.querySelector("#quickArcadePlayAgainButton").addEventListener("click", () => startQuickArcadeGame(quickArcadeState.gameId));
  elements.quickArcadeSummary.querySelector("#quickArcadeReturnButton").addEventListener("click", () => { renderGamesLabMenu(); showView("gamesView"); });
}

async function saveGameAttempt(data) {
  if (!auth.currentUser || currentProfile?.role !== "student" || data.previewOnly) return null;
  if (!arcadeIsOpenForCurrentPupil()) return null;
...
function startDebugSession() {
  ...
  const unlockState = gameUnlockState("debug-rescue");
  ...
  debugGameState = {
    level,
    mode,
    previewOnly: unlockState.previewOnly,
  ...
  elements.debugPreviewNotice.classList.toggle("hidden", !unlockState.previewOnly);
  elements.debugPreviewNotice.textContent = unlockState.previewOnly ? "Next-skill practice: this round will not be used to unlock the course pathway." : "";
...
function finishDebugSession() {
  ...
  elements.debugSummaryPanel.innerHTML = `<div class="game-result-hero"><span>🐟</span><div><h3>${debugGameState.rescued} fish rescued</h3><p>${debugGameState.score} points · ${debugGameState.timedOut} timed out</p></div></div>
    <p>${debugGameState.previewOnly ? "Next-skill practice complete. This round did not unlock the pathway." : "Your debugging skills and personalised review schedule have been updated."}</p>
    <div class="activity-actions"><button id="debugPlayAgainButton" type="button">Play again</button><button id="debugReturnGamesButton" type="button" class="secondary">Return to Arcade</button></div>`;
...
// ---------------- Revision Hub ----------------
function revisionStateSkillId(cardId) { return `card:${cardId}`; }
function revisionState(card) { return currentReviewStates.get(revisionStateSkillId(card.id)); }
function completedUnitIds() {
  return new Set([...currentProgress.entries()].filter(([, row]) => row.completed).map(([id]) => allActivities.find(a => a.id === id)?.unitId).filter(Boolean));
}
function eligibleRevisionCards() {
  const completed = completedUnitIds();
  return REVISION_CARDS.filter(card => completed.has(card.unitId));
}
function revisionDueInfo() {
  const pool = eligibleRevisionCards();
  const now = Date.now();
  const due = pool.filter(card => {
    const state = revisionState(card);
    return !state || timestampMillis(state.nextReviewAt) <= now;
  }).length;
  return { pool, due };
}
function renderRevisionHubCard() {
  if (!elements.revisionHubCard) return;
  const info = revisionDueInfo();
  const studiedUnits = new Set(info.pool.map(card => card.unitId)).size;
  elements.revisionHubDescription.textContent = info.pool.length
    ? `${info.due} card${info.due === 1 ? '' : 's'} ready for retrieval. Type your answer, compare it with the accepted wording, then rate yourself honestly.`
    : 'Complete an activity in a unit to unlock its approved revision cards.';
  elements.revisionHubMeta.innerHTML = `<span class="activity-chip">${info.pool.length} available cards</span><span class="activity-chip">${studiedUnits} studied unit${studiedUnits === 1 ? '' : 's'}</span><span class="activity-chip">Self-marked spacing</span>`;
  elements.openRevisionHubButton.disabled = info.pool.length === 0;
}
function revisionCardAttempts(cardId) {
  return currentReviewAttempts.filter(item => item.revisionCard && item.questionId === cardId);
}
function revisionWeakScore(card) {
  const attempts = revisionCardAttempts(card.id);
  if (!attempts.length) return 0.5;
  const ratings = attempts.map(a => Number(a.selfRating || a.confidence || 1));
  return 1 - ratings.reduce((a,b)=>a+b,0) / (ratings.length * 3);
}
function populateRevisionSetup() {
  const pool = eligibleRevisionCards();
  const availableAreas = AREAS.filter(area => pool.some(card => card.areaId === area.id));
  elements.revisionAreaFilter.innerHTML = `<option value="all">All studied areas</option>${availableAreas.map(area => `<option value="${area.id}">${escapeHtml(area.shortTitle)} — ${escapeHtml(area.title)}</option>`).join('')}`;
  const renderUnits = () => {
    const areaId = elements.revisionAreaFilter.value;
    const units = UNITS.filter(unit => pool.some(card => card.unitId === unit.id) && (areaId === 'all' || unit.areaId === areaId));
    elements.revisionTopicChoices.innerHTML = units.map(unit => `<label class="revision-topic-choice"><input type="checkbox" value="${unit.id}" checked> <span><strong>${escapeHtml(areaById(unit.areaId).shortTitle)} ${unit.number}</strong> ${escapeHtml(unit.title)}</span></label>`).join('');
  };
  elements.revisionAreaFilter.onchange = renderUnits;
  renderUnits();
  const info = revisionDueInfo();
  elements.revisionSetupSummary.innerHTML = `<strong>${info.pool.length}</strong> approved cards available · <strong>${info.due}</strong> due now`;
}
function chooseRevisionCards(mode, length, selectedUnits) {
  let pool = eligibleRevisionCards().filter(card => !selectedUnits.size || selectedUnits.has(card.unitId));
  const now = Date.now();
  if (mode === 'due') {
    const due = pool.filter(card => !revisionState(card) || timestampMillis(revisionState(card).nextReviewAt) <= now);
    if (due.length) pool = due;
  } else if (mode === 'weak') {
    pool = [...pool].sort((a,b) => revisionWeakScore(b) - revisionWeakScore(a));
  } else if (mode === 'exam') {
    const exam = pool.filter(card => card.examStyle);
    if (exam.length) pool = exam;
  }
  const unseen = pool.filter(card => !revisionCardAttempts(card.id).length);
  const seen = pool.filter(card => revisionCardAttempts(card.id).length).sort((a,b) => {
    const dueA = timestampMillis(revisionState(a)?.nextReviewAt) || 0;
    const dueB = timestampMillis(revisionState(b)?.nextReviewAt) || 0;
    return dueA - dueB || revisionWeakScore(b) - revisionWeakScore(a);
  });
  const mixed = mode === 'weak' ? pool : [...unseen.sort(()=>Math.random()-0.5), ...seen];
  return mixed.slice(0, Math.min(length, mixed.length));
}
function beginRevisionSession() {
  const selectedUnits = new Set([...elements.revisionTopicChoices.querySelectorAll('input:checked')].map(i => i.value));
  if (!selectedUnits.size) {
    elements.revisionSetupFeedback.textContent = 'Select at least one topic.';
    elements.revisionSetupFeedback.className = 'feedback error';
    return;
  }
  const queue = chooseRevisionCards(elements.revisionModeInput.value, Number(elements.revisionLengthInput.value), selectedUnits);
  if (!queue.length) {
    elements.revisionSetupFeedback.textContent = 'No cards match those choices yet.';
    elements.revisionSetupFeedback.className = 'feedback error';
    return;
  }
  revisionSession = { queue, index: 0, ratings: { secure:0, partial:0, not:0 }, retries:new Set(), currentRated:false, currentRating:null };
  elements.revisionSetupPanel.classList.add('hidden');
  elements.revisionCardPanel.classList.remove('hidden');
  elements.revisionSummaryPanel.classList.add('hidden');
  renderRevisionCard();
}
function renderRevisionCard() {
  const card = revisionSession?.queue[revisionSession.index];
  if (!card) return finishRevisionSession();
  const unit = unitById(card.unitId); const area = areaById(card.areaId);
  elements.revisionProgress.textContent = `Card ${revisionSession.index + 1} of ${revisionSession.queue.length}`;
  elements.revisionCardMeta.innerHTML = `<span class="area-badge area-${area.id}">${escapeHtml(area.shortTitle)}</span><span class="activity-chip">Unit ${unit.number}: ${escapeHtml(unit.title)}</span>`;
  elements.revisionPrompt.textContent = card.prompt;
  elements.revisionAnswerInput.value = '';
  elements.revisionComparePanel.classList.add('hidden');
  elements.revisionRatingButtons.classList.add('hidden');
  elements.revisionNextActions?.classList.add('hidden');
  elements.compareRevisionAnswerButton.classList.remove('hidden');
  elements.revisionAcceptedAnswer.textContent = '';
  elements.revisionMarkPoints.innerHTML = '';
  elements.revisionCardFeedback.className = 'feedback hidden';
  revisionSession.currentRated = false;
  revisionSession.currentRating = null;
  [elements.revisionNotSecureButton, elements.revisionPartlySecureButton, elements.revisionSecureButton].forEach(button => { button.disabled = false; button.classList.remove('selected-rating'); });
  revisionQuestionStartedAt = Date.now();
  setTimeout(() => elements.revisionAnswerInput.focus(), 0);
}
function compareRevisionAnswer() {
  const card = revisionSession?.queue[revisionSession.index];
  if (!card) return;
  if (!elements.revisionAnswerInput.value.trim()) {
    elements.revisionCardFeedback.textContent = 'Type what you can remember before comparing.';
    elements.revisionCardFeedback.className = 'feedback error';
    return;
  }
  elements.revisionOwnAnswerMirror.textContent = elements.revisionAnswerInput.value;
  elements.revisionAcceptedAnswer.textContent = card.answer;
  const points = card.points?.length ? card.points : card.answer.split(/\n+/).map(x=>x.replace(/^[-•]\s*/, '').trim()).filter(Boolean).slice(0,6);
  elements.revisionMarkPoints.innerHTML = points.length ? `<h4>Check what you included</h4>${points.map((point,index)=>`<label class="revision-point"><input type="checkbox" value="${index}"> <span>${escapeHtml(point)}</span></label>`).join('')}` : '<p>Compare the meaning carefully rather than looking for identical wording.</p>';
  elements.revisionComparePanel.classList.remove('hidden');
  elements.revisionRatingButtons.classList.remove('hidden');
  elements.compareRevisionAnswerButton.classList.add('hidden');
  elements.revisionCardFeedback.textContent = 'Now rate your retrieval honestly. The accepted answer is not automatically treated as a word-for-word requirement.';
  elements.revisionCardFeedback.className = 'feedback info';
}
function revisionStateData(old, card, rating) {
  const oldStage = Number(old?.stage || 0);
  let stage, days;
  if (rating === 1) { stage = Math.max(0, oldStage - 1); days = 1; }
  else if (rating === 2) { stage = Math.max(1, oldStage); days = 2; }
  else { stage = Math.min(SPACED_INTERVAL_DAYS.length - 1, oldStage + 1); days = SPACED_INTERVAL_DAYS[stage]; }
  return { userId:auth.currentUser.uid, classId:learningScopeId(), courseId:COURSE_ID, skillId:revisionStateSkillId(card.id), stage, nextReviewAt:new Date(Date.now()+days*86400000), lastReviewedAt:serverTimestamp(), correctCount:Number(old?.correctCount||0)+(rating===3?1:0), incorrectCount:Number(old?.incorrectCount||0)+(rating===1?1:0), consecutiveCorrect:rating===3?Number(old?.consecutiveCorrect||0)+1:0, lastCorrect:rating===3, selfRating:rating, updatedAt:serverTimestamp() };
}
async function rateRevisionCard(rating) {
  const card = revisionSession?.queue[revisionSession.index];
  if (!card || revisionSession.currentRated) return;
  const ratingButtons = [elements.revisionNotSecureButton, elements.revisionPartlySecureButton, elements.revisionSecureButton];
  const selectedButton = rating === 1 ? elements.revisionNotSecureButton : rating === 2 ? elements.revisionPartlySecureButton : elements.revisionSecureButton;
  ratingButtons.forEach(button => { button.disabled = true; button.classList.toggle('selected-rating', button === selectedButton); });

  revisionSession.currentRated = true;
  revisionSession.currentRating = rating;
  const responseSeconds = Math.max(1, Math.round((Date.now()-revisionQuestionStartedAt)/1000));
  const attemptRef = doc(collection(db,'reviewAttempts'));
  const skillId = revisionStateSkillId(card.id);
  const old = currentReviewStates.get(skillId) || {};
  const state = revisionStateData(old, card, rating);

  // Update this session immediately so slow or unavailable cloud saving never traps the pupil on a card.
  currentReviewStates.set(skillId,{...old,...state});
  currentReviewAttempts.push({questionId:card.id,revisionCard:true,selfRating:rating,confidence:rating,selectedAnswer:elements.revisionAnswerInput.value,reviewedAt:new Date(),skillIds:[skillId]});
  if (rating===3) revisionSession.ratings.secure+=1;
  else if (rating===2) revisionSession.ratings.partial+=1;
  else {
    revisionSession.ratings.not+=1;
    if(!revisionSession.retries.has(card.id)){
      revisionSession.retries.add(card.id);
      revisionSession.queue.splice(Math.min(revisionSession.queue.length, revisionSession.index+4), 0, card);
    }
  }

  const ratingLabel = rating === 3 ? 'Secure' : rating === 2 ? 'Partly secure' : 'Not secure';
  elements.revisionCardFeedback.innerHTML = `<strong>${ratingLabel} recorded.</strong> Review the comparison, then press <strong>Next flashcard</strong>.`;
  elements.revisionCardFeedback.className = 'feedback success';
  elements.revisionNextButton.textContent = revisionSession.index + 1 >= revisionSession.queue.length ? 'Finish session →' : 'Next flashcard →';
  elements.revisionNextActions.classList.remove('hidden');
  setTimeout(() => elements.revisionNextButton.focus(), 0);

  const batch=writeBatch(db);
  batch.set(attemptRef,{userId:auth.currentUser.uid,classId:learningScopeId(),courseId:COURSE_ID,questionId:card.id,areaId:card.areaId,unitId:card.unitId,moduleTitle:unitById(card.unitId)?.title||'',skillIds:[skillId],selectedAnswer:elements.revisionAnswerInput.value,correct:rating===3,confidence:rating,selfRating:rating,revisionCard:true,responseTimeSeconds,retry:false,reviewedAt:serverTimestamp()});
  batch.set(doc(db,'reviewStates',reviewStateDocumentId(skillId)),state,{merge:Boolean(old && Object.keys(old).length)});

  // Save in the background. Navigation is deliberately not blocked by Firestore.
  batch.commit().catch(error => {
    console.error(error);
    if (revisionSession?.queue[revisionSession.index]?.id === card.id) {
      elements.revisionCardFeedback.innerHTML = `<strong>${ratingLabel} recorded for this session.</strong> Cloud saving is taking longer than expected, but you can still press <strong>Next flashcard</strong>.`;
      elements.revisionCardFeedback.className = 'feedback info';
    }
  });
}
function nextRevisionCard() {
  if (!revisionSession?.currentRated) return;
  revisionSession.index += 1;
  renderRevisionCard();
}
function finishRevisionSession() {
  if(!revisionSession) return;
  elements.revisionCardPanel.classList.add('hidden'); elements.revisionSummaryPanel.classList.remove('hidden'); elements.revisionProgress.textContent='Complete';
  elements.revisionSummaryPanel.innerHTML=`<div class="revision-summary-hero"><h3>Revision session complete</h3><p>Your self-ratings have updated the spaced schedule.</p></div><div class="stats-grid compact">${statCard(revisionSession.ratings.secure,'Secure')}${statCard(revisionSession.ratings.partial,'Partly secure')}${statCard(revisionSession.ratings.not,'Not secure')}</div><div class="activity-actions"><button id="anotherRevisionButton" type="button">Choose another session</button><button id="returnRevisionDashboardButton" type="button" class="secondary">Return to dashboard</button></div>`;
  elements.revisionSummaryPanel.querySelector('#anotherRevisionButton').addEventListener('click',()=>{elements.revisionSummaryPanel.classList.add('hidden');elements.revisionSetupPanel.classList.remove('hidden');populateRevisionSetup();});
  elements.revisionSummaryPanel.querySelector('#returnRevisionDashboardButton').addEventListener('click',async()=>{showView('pupilView');await loadPupilDashboard();});
}
function openRevisionHub() {
  populateRevisionSetup(); elements.revisionSetupPanel.classList.remove('hidden'); elements.revisionCardPanel.classList.add('hidden'); elements.revisionSummaryPanel.classList.add('hidden'); elements.revisionSetupFeedback.className='feedback hidden'; showView('revisionView');
}
...
function startLogicSession() {
  ...
  const unlockState = gameUnlockState('logic-showdown');
  const unlocked = unlockState.unlocked;
  const total = mode === 'truth-table' ? 4 : 6;
  logicGameState = { level, mode, previewOnly: unlockState.previewOnly, total, index: 0, correct: 0, selectedAnswer: null, startedAt: Date.now(), usedIds: new Set() };
  ...
  elements.logicPreviewNotice.classList.toggle('hidden', !unlockState.previewOnly);
  elements.logicPreviewNotice.textContent = unlockState.previewOnly ? 'Next-skill practice: this round will not be used to unlock the course pathway.' : '';
  renderLogicQuestion();
}
...
function finishLogicSession() {
  ...
  elements.logicSummaryPanel.innerHTML = `<div class="game-result-hero"><span>⚖️</span><div><h3>${badge}</h3><p>${logicGameState.correct}/${logicGameState.total} correct · ${percentage}%</p></div></div>
    <p>${logicGameState.previewOnly ? 'Next-skill practice complete. This round did not unlock the pathway.' : 'Your logical-operator mastery and review schedule have been updated.'}</p>
    <div class="activity-actions"><button id="logicPlayAgainButton" type="button">Play again</button><button id="logicReturnGamesButton" type="button" class="secondary">Return to Arcade</button></div>`;
...
// Revision Hub navigation and controls
elements.openRevisionHubButton.addEventListener('click', openRevisionHub);
elements.backFromRevisionButton.addEventListener('click', async () => { showView('pupilView'); await loadPupilDashboard(); });
elements.startRevisionSessionButton.addEventListener('click', beginRevisionSession);
elements.compareRevisionAnswerButton.addEventListener('click', compareRevisionAnswer);
elements.revisionNotSecureButton.addEventListener('click', () => void rateRevisionCard(1));
elements.revisionPartlySecureButton.addEventListener('click', () => void rateRevisionCard(2));
elements.revisionSecureButton.addEventListener('click', () => void rateRevisionCard(3));
elements.revisionNextButton.addEventListener('click', nextRevisionCard);

// Computing Arcade navigation and controls
elements.openGamesLabButton.addEventListener('click', () => {
  if (!arcadeIsOpenForCurrentPupil()) {
    showMessage('The Computing Arcade is closed for your class just now.', 'info');
    return;
  }
  arcadePracticeMode = 'unlocked';
  showView('gamesView');
  renderGamesLabMenu();
});
elements.backFromGamesButton.addEventListener('click', async () => { showView('pupilView'); await loadPupilDashboard(); });
elements.arcadeUnlockedModeButton?.addEventListener('click', () => setArcadePracticeMode('unlocked'));
elements.arcadeNextModeButton?.addEventListener('click', () => setArcadePracticeMode('next'));
Object.keys(ARCADE_GAME_CONFIG).forEach(gameId => {
  elements[`arcade-${gameId}-button`]?.addEventListener('click', () => startQuickArcadeGame(gameId));
});
elements.backFromQuickArcadeButton?.addEventListener('click', () => { renderGamesLabMenu(); showView('gamesView'); });
elements.submitQuickArcadeAnswerButton?.addEventListener('click', () => void submitQuickArcadeAnswer());
elements.nextQuickArcadeQuestionButton?.addEventListener('click', nextQuickArcadeQuestion);
elements.openDebugGameButton.addEventListener('click', () => {
  if (!gameUnlockState('debug-rescue').unlocked) return;
...
elements.openLogicGameButton.addEventListener('click', () => {
  if (!gameUnlockState('logic-showdown').unlocked) return;
