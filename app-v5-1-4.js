...
function showDebugGameOver() {
  ...
  const previousImage = elements.debugGameOverGif;
  const gifPath = previousImage.dataset.gifSrc || "game_over.gif";

  // Creating a completely new image element guarantees that an animated GIF
  // starts again at frame one, even when the browser has cached the file.
  const freshImage = document.createElement("img");
  freshImage.id = "debugGameOverGif";
  freshImage.dataset.gifSrc = gifPath;
  freshImage.alt = previousImage.alt || "Animated shark game-over sequence";

  freshImage.addEventListener("error", () => {
    console.error("The game-over GIF could not be loaded:", gifPath);
    freshImage.replaceWith(
      Object.assign(document.createElement("p"), {
        id: "debugGameOverGif",
        className: "game-over-load-error",
        textContent: "The shark animation could not be loaded. Check that game_over.gif is in the GitHub repository."
      })
    );
  }, { once: true });

  previousImage.replaceWith(freshImage);
  elements.debugGameOverGif = freshImage;

  elements.debugGameOverOverlay.classList.remove("hidden");

  // Add the source only after the visible overlay and new image exist.
  // The unique value prevents Edge from reusing an already-finished animation.
  requestAnimationFrame(() => {
    freshImage.src = `${gifPath}?play=${Date.now()}`;
  });

  // The original animation has 43 frames and lasts about 4.3 seconds.
