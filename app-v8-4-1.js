...
console.info(`Nat 5 Computing v${APP_VERSION}: Mastery Edition + Computing Arcade + simplified Google classroom sign-in`);
...
async function routeSignedInUser(user) {
  try {
    ...
    if (isSchoolPupil && !providerIds.has("google.com")) {
      showMessage("Classroom pupils now sign in with Google. Choose Continue with Google using the account you joined the class with.", "info");
      await signOut(auth);
      return;
    }
...
function friendlyAuthMessage(error, fallback) {
  ...
  if (code.includes("operation-not-allowed")) return "Email/password sign-in is not enabled in Firebase Authentication. It must stay enabled for Personal Study licences.";
...
elements.pupilGoogleLoginButton.addEventListener("click", async () => {
  ...
  try {
    ...
    if (!profileSnapshot.exists()) {
      await signOut(auth);
      showMessage("That Google account has not joined Nat 5 Computing yet. Choose Join a class, enter your class code and join with Google first.", "info");
      return;
    }
...
elements.individualLoginForm.addEventListener("submit", async event => {
  ...
  try {
    ...
    if (!profileSnapshot.exists() || profileSnapshot.data().accountType !== "individual") {
      await signOut(auth);
      showMessage("Classroom pupils sign in with Google. Email + password sign-in is only for Personal Study licences.", "info");
      return;
    }
    accountSetupInProgress = false;
...
elements.pupilForgotPasswordButton.addEventListener("click", async () => {
  ...
  try {
    ...
    showMessage("If that email address has a Personal Study email/password account, Firebase has accepted the password-reset request. Check your inbox and junk/spam folder. Classroom pupils should use Continue with Google instead.", "success");
  } catch (error) {
    console.error(error);
...
elements.pupilRegisterForm.addEventListener("submit", event => {
  event.preventDefault();
  elements.pupilGoogleRegisterButton.click();
});
