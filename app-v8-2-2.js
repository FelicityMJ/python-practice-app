...
console.info(`Nat 5 Computing v${APP_VERSION}: ${AREAS.filter(area => area.active).length} active areas, ${UNITS.length} units, ${ACTIVITIES.length} pathway activities, ${SPACED_QUESTIONS.length} spaced questions, ${REVISION_CARDS.length} revision cards, ${FLUENCY_SPRINTS.length} six-attempt sprints`);
...
function showView(viewId) {
  ["authView", "teacherView", "pupilView", "gamesView", "debugGameView", "logicGameView", "spacedView", "revisionView", "peerReviewView", "activityView", "challengeView"].forEach(id => {
      ...
      row.innerHTML = `
        ...
        <div class="activity-main">
          ...
          <div class="activity-meta">
            ...
            ${activity.type === "fluency-sprint" ? '<span class="activity-chip fluency-chip">6 attempts</span>' : ""}
...
function renderLicenceStatus() {
  ...
  if (currentEntitlement?.permanent === true) {
    elements.licenceStatusText.textContent = "Full Course and Exam Accelerator are permanently available on this account.";
    ...
    elements.licenceStatusText.textContent = "Full Course and Exam Accelerator are available through your individual licence.";
    elements.licenceStatusBadge.textContent = "Active";
  }
}
...
function renderLearningRouteControls() {
  ...
  elements.learningRouteNotice.innerHTML = accelerator
    ...
    : "<strong>Full Course:</strong> work through the carefully sequenced pathway. Required activities unlock in order and build secure understanding before exam practice.";
}
...
async function saveActiveFluencyProgress({ complete = false, countAttempt = false, correct = false } = {}) {
  ...
  await saveGenericProgress(activity, updates);
  codeIsDirty = false;
  clearScheduledAutoSave();
...
function renderSddFluencyAttempt() {
  ...
  elements.challengeInstructions.innerHTML = `
    ...
    <div class="activity-meta"><span class="activity-chip">Attempt ${state.index + 1} of ${tasks.length}</span><span class="activity-chip">${support}</span></div>
    <p class="fluency-challenge-prompt"><strong>${escapeHtml(task.prompt)}</strong></p>
    ${language === "python" ? '<p class="editor-help-line">Type Python below. Run and Visualise are available before you check your answer.</p>' : `<p class="editor-help-line">${language === "pseudocode" ? "Type the pseudocode below. Pseudocode cannot be executed by Python, so Run and Visualise are paused for this attempt." : "Type your written answer below."}</p>`}`;
  ...
  elements.challengeStatus.textContent = state.correct[state.index] ? "Attempt complete" : "In progress";
...
async function checkActiveFluencyAttempt() {
  ...
  if (language === "python") {
    ...
    if (execution.error) {
      ...
      elements.feedbackBox.textContent = "Python stopped with an error. Repair it, then check the attempt again.";
      elements.feedbackBox.className = "feedback error";
  ...
  const correct = fluencyTaskCorrect(task, response);
  ...
  elements.challengeStatus.textContent = "Attempt complete";
  ...
  nextButton.textContent = state.index >= activity.tasks.length - 1 ? "Complete sprint →" : "Next attempt →";
  nextButton.addEventListener("click", async () => {
    nextButton.disabled = true;
    if (state.index >= activity.tasks.length - 1) {
      await saveActiveFluencyProgress({ complete: true });
      elements.feedbackBox.innerHTML = "<strong>All six attempts are complete.</strong> This practical skill has now been practised across the full guided sequence.";
      elements.feedbackBox.className = "feedback success";
...
function renderFluencySprint(activity, progress) {
  ...
  const tasks = activity.tasks || [];
  elements.activityContent.innerHTML = `<p>This activity gives six deliberate attempts at the same new skill. Support fades as you progress.</p><div class="fluency-progress-strip">${tasks.map((_, index) => `<span data-fluency-dot="${index}">${index + 1}</span>`).join("")}</div>`;
  elements.activityInteraction.innerHTML = `<section class="fluency-shell"><div id="fluencyTaskMeta" class="activity-meta"></div><h3 id="fluencyTaskPrompt"></h3><div id="fluencyTaskInput"></div><div id="fluencyHint" class="hint-box hidden"></div><div class="activity-actions"><button id="checkFluencyButton" type="button">Check attempt</button><button id="fluencyHintButton" type="button" class="secondary">Show support</button><button id="nextFluencyButton" type="button" class="secondary hidden">Next attempt</button></div><div id="fluencyFeedback" class="feedback hidden" aria-live="polite"></div></section>`;
  ...
  const saveState = async (completed = false) => {
    await saveGenericProgress(activity, { fluency: state, completed }, completed ? { correct: true } : {});
  };
  const render = () => {
    const task = tasks[state.index];
    const support = state.index <= 1 ? 'Worked pattern available' : state.index <= 3 ? 'Compact reminder' : state.index === 4 ? 'Generate from the prompt' : 'Near transfer';
    meta.innerHTML = `<span class="activity-chip">Attempt ${state.index + 1} of 6</span><span class="activity-chip">${support}</span>`;
  ...
  checkButton.addEventListener('click', async () => {
    ...
    if (correct) {
      ...
      nextButton.textContent = state.index === tasks.length - 1 ? 'Complete sprint' : 'Next attempt →';
    } else {
      hint.classList.remove('hidden');
    }
  });
  hintButton.addEventListener('click', () => hint.classList.toggle('hidden'));
  nextButton.addEventListener('click', async () => {
    if (!state.correct[state.index]) return;
    if (state.index >= tasks.length - 1) {
      await saveState(true);
      setGenericFeedback('All six attempts are correct. Fluency sprint complete.', 'success');
      setGenericStatus(activity);
      renderRelatedPractice(activity, elements.activityRelatedPractice);
      return;
    }
