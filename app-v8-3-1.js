...
console.info(`Nat 5 Computing v${APP_VERSION}: Mastery Edition + secure teacher pupil-password reset`);

const firebaseApp = initializeApp(firebaseConfig);
const auth = getAuth(firebaseApp);
const db = getFirestore(firebaseApp);
const functions = getFunctions(firebaseApp, "europe-west1");
const resetPupilPasswordCallable = httpsCallable(functions, "resetPupilPassword");
...
function showView(viewId) {
  ["authView", "teacherView", "pupilView", "dailyMasteryView", "masteryDiagnosticView", "gamesView", "debugGameView", "logicGameView", "spacedView", "revisionView", "peerReviewView", "activityView", "challengeView"].forEach(id => {
...
elements.individualLoginForm?.addEventListener("submit", async event => {
  ...
  clearMessage();
  const email = elements.individualLoginEmail.value.trim().toLowerCase();
  const password = elements.individualLoginPassword.value;
  if (!email || !password) {
    showMessage("Enter the email address and password for the individual account.", "error");
    return;
  }
  try {
    ...
    console.error(error);
    showMessage("The individual email address or password was not recognised.", "error");
  }
});

elements.individualResetPasswordButton?.addEventListener("click", async () => {
  clearMessage();
  const email = elements.individualLoginEmail.value.trim().toLowerCase();
  if (!email) {
    showMessage("Enter the individual account email address first.", "error");
    return;
  }
  try {
    await sendPasswordResetEmail(auth, email);
    showMessage("A password-reset email has been sent if that individual account exists.", "success");
  } catch (error) {
    console.error(error);
    showMessage("The password-reset email could not be sent.", "error");
  }
});
...
function generateTemporaryPupilPassword() {
  ...
  const required = [uppercase, lowercase, digits, symbols].map(group => group[randomIndex(group.length)]);
  while (required.length < 12) required.push(all[randomIndex(all.length)]);
...
function pupilLoginDetailsText(classItem, member, password) {
  return `Nat 5 Computing pupil sign in\nClass code: ${classItem.joinCode}\nUsername: ${member.username}\nPassword: ${password}`;
}

function wirePupilPasswordReset(classItem, member, openImmediately = false) {
  const panel = elements.pupilInsightPanel.querySelector("#pupilPasswordResetPanel");
  const toggleButton = elements.pupilInsightPanel.querySelector("#openPupilPasswordResetButton");
  const cancelButton = elements.pupilInsightPanel.querySelector("#cancelPupilPasswordResetButton");
  const form = elements.pupilInsightPanel.querySelector("#pupilPasswordResetForm");
  const passwordInput = elements.pupilInsightPanel.querySelector("#pupilNewPassword");
  const confirmInput = elements.pupilInsightPanel.querySelector("#pupilNewPasswordConfirm");
  const generateButton = elements.pupilInsightPanel.querySelector("#generatePupilPasswordButton");
  const status = elements.pupilInsightPanel.querySelector("#pupilPasswordResetStatus");
  const result = elements.pupilInsightPanel.querySelector("#pupilPasswordResetResult");

  if (!panel || !toggleButton || !form) return;
  ...
  toggleButton.addEventListener("click", () => setOpen(panel.classList.contains("hidden")));
  ...
  generateButton?.addEventListener("click", () => {
    const password = generateTemporaryPupilPassword();
    passwordInput.value = password;
    confirmInput.value = password;
    status.textContent = "Temporary password generated. Save it to apply the change.";
  ...
  form.addEventListener("submit", async event => {
    ...
    if (newPassword.length > 128) {
      status.textContent = "Use no more than 128 characters.";
      status.className = "save-status error";
      return;
    }
    ...
    status.textContent = "Resetting password securely…";
    status.className = "save-status saving";

    try {
      const response = await resetPupilPasswordCallable({
        classId: classItem.id,
        pupilUid: member.userId,
        newPassword
      });
      if (!response?.data?.ok) throw new Error("The reset did not complete.");
      status.textContent = "Password reset successfully.";
      status.className = "save-status saved";
      const details = pupilLoginDetailsText(classItem, member, newPassword);
      result.innerHTML = `
        <strong>New sign-in details for ${escapeHtml(member.displayName)}</strong>
        <div class="pupil-login-credential-grid">
          <span>Class code</span><code>${escapeHtml(classItem.joinCode)}</code>
          <span>Username</span><code>${escapeHtml(member.username)}</code>
          <span>New password</span><code>${escapeHtml(newPassword)}</code>
        </div>
        <div class="card-actions">
          <button type="button" class="secondary small" id="copyPupilLoginDetailsButton">Copy login details</button>
        </div>
        <p class="help-text">The password is shown here so you can give it to the pupil. It is not saved in the class database and will disappear when you close or reload this profile.</p>`;
      result.classList.remove("hidden");
      const copyButton = result.querySelector("#copyPupilLoginDetailsButton");
      copyButton?.addEventListener("click", async () => {
        try {
          await navigator.clipboard.writeText(details);
          copyButton.textContent = "Copied";
        } catch (_) {
          copyButton.textContent = "Copy failed — select the details above";
        }
      });
      form.reset();
    } catch (error) {
      console.error("Pupil password reset failed", error);
      const code = String(error?.code || "");
      const message = code.includes("permission-denied")
        ? "You are not authorised to reset this pupil's password."
        : code.includes("unauthenticated")
          ? "Your teacher session has expired. Sign in again and retry."
          : code.includes("invalid-argument")
            ? (error?.message || "Check the password and try again.")
            : "The password could not be reset. Check that the Firebase reset function has been deployed, then try again.";
...
function showPupilInsight(classItem, member, options = {}) {
  const metrics = pupilMetrics(classItem, member);
  ...
  elements.pupilInsightPanel.innerHTML = `
    <div class="panel-heading">
      ...
      <div class="card-actions profile-heading-actions">
        <button id="openPupilPasswordResetButton" type="button" class="secondary small" aria-expanded="false">Reset password</button>
        <button id="closePupilInsightButton" type="button" class="secondary small">Close profile</button>
      </div>
    </div>
    <section id="pupilPasswordResetPanel" class="profile-section pupil-password-reset-panel hidden">
      <div class="panel-heading compact-heading">
        <div>
          <h5>Reset pupil password</h5>
          <p class="help-text">Set a temporary password for ${escapeHtml(member.displayName)}. The pupil keeps the same class, username and progress.</p>
        </div>
      </div>
      <form id="pupilPasswordResetForm" autocomplete="off">
        <div class="password-reset-grid">
          <label>New password
            <input id="pupilNewPassword" type="password" minlength="8" maxlength="128" autocomplete="off" required>
          </label>
          <label>Confirm password
            <input id="pupilNewPasswordConfirm" type="password" minlength="8" maxlength="128" autocomplete="off" required>
          </label>
        </div>
        <div class="card-actions">
          <button id="generatePupilPasswordButton" type="button" class="secondary small">Generate temporary password</button>
          <button type="submit" class="small">Save new password</button>
          <button id="cancelPupilPasswordResetButton" type="button" class="secondary small">Cancel</button>
        </div>
        <p id="pupilPasswordResetStatus" class="save-status" aria-live="polite"></p>
      </form>
      <div id="pupilPasswordResetResult" class="pupil-password-reset-result hidden" aria-live="polite"></div>
    </section>
  ...
  wirePupilPasswordReset(classItem, member, options.openPasswordReset === true);
  ...
  classItem.members
    ...
    .forEach(member => {
      ...
      row.innerHTML = `
        ...
        <td><div class="pupil-row-actions"><button type="button" class="secondary small open-pupil-profile">Open profile</button><button type="button" class="secondary small reset-pupil-password">Reset password</button></div></td>`;
      ...
      row.querySelector(".reset-pupil-password").addEventListener("click", () => showPupilInsight(classItem, member, { openPasswordReset: true }));
