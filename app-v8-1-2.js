...
console.info(`Python Practice v${APP_VERSION}: ${AREAS.filter(area => area.active).length} active areas, ${UNITS.length} units, ${ACTIVITIES.length} pathway activities, ${SPACED_QUESTIONS.length} spaced questions, ${REVISION_CARDS.length} revision cards, ${FLUENCY_SPRINTS.length} six-attempt sprints`);
...
function revisionStateData(old, card, rating) {
  ...
  return { userId:auth.currentUser.uid, classId:currentProfile.classId, skillId:revisionStateSkillId(card.id), stage, nextReviewAt:new Date(Date.now()+days*86400000), lastReviewedAt:serverTimestamp(), correctCount:Number(old?.correctCount||0)+(rating===3?1:0), incorrectCount:Number(old?.incorrectCount||0)+(rating===1?1:0), consecutiveCorrect:rating===3?Number(old?.consecutiveCorrect||0)+1:0, lastCorrect:rating===3, selfRating:rating, updatedAt:serverTimestamp() };
}
