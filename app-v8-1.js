import { REVISION_CARDS, FLUENCY_SPRINTS } from "./revision-fluency-v8.js";
...
const ACTIVITY_TYPE_LABELS = {
  ...
  "fluency-sprint": "Six-attempt fluency"
};
...
function renderSddFluencyAttempt() {
  ...
  elements.challengeTopic.textContent = `Six-attempt fluency · Unit ${unit?.number || ""} ${unit?.title || ""}`;
...
async function checkActiveFluencyAttempt() {
  ...
  nextButton.addEventListener("click", async () => {
    ...
    if (state.index >= activity.tasks.length - 1) {
      ...
      elements.feedbackBox.innerHTML = "<strong>All six attempts are complete.</strong> This practical skill has now been practised across the full fluency sequence.";
      elements.feedbackBox.className = "feedback success";
...
function renderFluencySprint(activity, progress) {
  ...
  const tasks = activity.tasks || [];
  elements.activityContent.innerHTML = `<p>This sprint gives six deliberate attempts at the same new skill. Support fades as you progress.</p><div class="fluency-progress-strip">${tasks.map((_, index) => `<span data-fluency-dot="${index}">${index + 1}</span>`).join("")}</div>`;
