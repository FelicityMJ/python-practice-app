...
function acceleratorActivitiesForUnit(unitId) {
  ...
  const addFirst = types => {
    const item = activities.find(activity => types.includes(activity.type) && !selected.some(row => row.id === activity.id));
    if (item) selected.push(item);
  };
  addFirst(["lesson", "video"]);
  addFirst(["predict", "visualiser", "code", "debug", "sql", "binary-builder", "erd-builder"]);
  addFirst(["quiz", "fluency-sprint"]);
  addFirst(["exam-style", "official-paper"]);
  activities.filter(activity => activity.required !== false).forEach(activity => {
    if (selected.length < 4 && !selected.some(row => row.id === activity.id)) selected.push(activity);
  });
  return selected.length ? selected : activities.slice(0, 4);
}
...
function renderLearningRouteControls() {
  ...
  elements.learningRouteNotice.innerHTML = accelerator
    ? "<strong>Exam Accelerator:</strong> every topic remains available. Each unit keeps the highest-value explanation, application, retrieval and exam task, while progress continues to count towards the same mastery profile."
