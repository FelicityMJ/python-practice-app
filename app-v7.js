...
const DDD_ACTIVITIES = [
  ...
  },
  {
    ...
    "unitId": "ddd-erd-04",
    "order": 5.5,
    "title": "Extra practice: Build a library ERD",
    "skills": [
      "erd",
      "entity",
      "attribute",
    ...
    "starterState": {
      ...
      "oneToManyVerb": "appears in",
      "manyToOneVerb": "refers to"
    },
    "expected": {
      "oneEntity": [
        "book"
...
ACTIVITIES.push(...[
  {
    ...
    "required": true,
    "estimatedMinutes": 6,
    "contentHtml": "<p>Computers store data using two binary digits: <strong>0</strong> and <strong>1</strong>. In an 8-bit positive integer, the place values are:</p><div class=\"binary-place-strip\"><span>128</span><span>64</span><span>32</span><span>16</span><span>8</span><span>4</span><span>2</span><span>1</span></div><p>A 1 means include that place value. A 0 means do not include it.</p>",
    "checkpoint": {
  ...
  },
  {
    ...
    "unitId": "cs-binary-01",
    "order": 2,
    "title": "Build: Read an 8-bit positive integer",
    "skills": [
      "binary",
      "positive-integers",
      "place-values"
  ...
  },
  {
    ...
    "required": true,
    "estimatedMinutes": 6,
    "contentHtml": "<p>To convert binary to denary, add the place values with a 1.</p><pre class=\"lesson-code\">00101101 = 32 + 8 + 4 + 1 = 45</pre><p>To convert denary to binary, choose place values from largest to smallest and write 1 when each is used.</p>",
    "checkpoint": {
      "prompt": "What is 00001010 in denary?",
      "options": [
        "10",
        "8",
        "12"
  ...
  },
  {
    ...
    "unitId": "cs-conversion-02",
    "order": 2,
    "title": "Build: Convert denary 45 to binary",
    "skills": [
      "binary",
      "denary",
      "conversion"
  ...
  },
  {
    ...
    "unitId": "cs-conversion-02",
    "order": 3,
    "title": "Build: Convert binary to denary",
    "skills": [
      "binary",
      "denary",
      "conversion"
  ...
  },
  {
    ...
    "questions": [
      {
        "prompt": "What is 11111111 in denary?",
        "options": [
          "255",
          "256",
          "128"
      ...
      {
        "prompt": "What is 00010001 in denary?",
        "options": [
          "17",
          "16",
          "18"
  ...
  },
  {
    ...
    "markingPoints": [
      ...
      "The correct 8-bit result is 01001011."
    ],
    "modelAnswer": "75 = **64 + 8 + 2 + 1**, so the 8-bit binary value is **01001011**."
  },
  {
    "id": "CS-03-01",
    "areaId": "cs",
...
SPACED_QUESTIONS.push(...[
  ...
  },
  {
    ...
    "areaId": "cs",
    "unitId": "cs-binary-01",
    "prompt": "What is 00000101 in denary?",
    "options": [
      "5",
      "6",
      "4"
  ...
  },
  {
    ...
    "areaId": "cs",
    "unitId": "cs-conversion-02",
    "prompt": "What is 00100000 in denary?",
    "options": [
      "32",
      "16",
      "64"
  ...
  },
  {
    ...
    "areaId": "cs",
    "unitId": "cs-conversion-02",
    "prompt": "What is 01000001 in denary?",
    "options": [
      "65",
      "64",
      "66"
  ...
  },
  {
    ...
    "areaId": "cs",
    "unitId": "cs-conversion-02",
    "prompt": "What is 10000000 in denary?",
    "options": [
      "128",
      "127",
      "256"
...
function renderBinaryBuilder(activity, progress) {
  ...
  elements.activityContent.innerHTML = `<div class="instructions"><p>${activity.mode === "denary-to-binary"
    ? `Set the eight bits to represent the denary value <strong>${activity.target}</strong>.`
    : `Read the supplied eight-bit pattern and enter its denary value.`}</p></div>`;
  elements.activityInteraction.innerHTML = `
    <div class="binary-builder" data-mode="${escapeHtml(activity.mode)}">
      <div class="binary-place-values">${placeValues.map(value => `<span>${value}</span>`).join("")}</div>
      <div class="binary-bits">${bits.map((bit, index) => `<button type="button" data-bit-index="${index}" ${activity.mode === "binary-to-denary" ? "disabled" : ""}>${bit}</button>`).join("")}</div>
      <div class="binary-total">Current denary value: <strong id="binaryCurrentTotal">0</strong></div>
      ${activity.mode === "binary-to-denary" ? `<label>Your denary answer <input id="binaryDenaryAnswer" type="number" min="0" max="255" value="${escapeHtml(progress.binaryAnswer ?? "")}"></label>` : ""}
  ...
  const totalElement = elements.activityInteraction.querySelector("#binaryCurrentTotal");
  const bitButtons = [...elements.activityInteraction.querySelectorAll("[data-bit-index]")];
  const update = () => {
    const total = bits.reduce((sum, bit, index) => sum + bit * placeValues[index], 0);
    ...
    bitButtons.forEach((button, index) => {
      ...
      button.setAttribute("aria-label", `${placeValues[index]} place: ${bits[index]}`);
    });
    return total;
  };
  ...
  elements.activityInteraction.querySelector("#checkBinaryButton").addEventListener("click", async () => {
    ...
    const answer = activity.mode === "binary-to-denary" ? Number(answerInput.value) : total;
    if (activity.mode === "binary-to-denary" && answerInput.value === "") return setGenericFeedback("Enter a denary answer first.", "error");
    const correct = answer === Number(activity.target);
    await saveGenericProgress(activity, { binaryBits: bits, binaryAnswer: answer }, { countAttempt: true, correct });
    if (correct) {
      setGenericFeedback(`Correct. The value is ${activity.target}.`, "success");
      setGenericStatus(activity);
      renderRelatedPractice(activity, elements.activityRelatedPractice);
    } else {
      setGenericFeedback("Not yet. Add the place values whose bits are 1, or adjust the selected bits.", "error");
    }
  });
  update();
}
...
function renderErdBuilder(activity, progress) {
  ...
  const renderTable = () => {
    ...
    canvas.innerHTML = `<div class="erd-table-diagram">${ordered.map((entity,index) => `<div class="erd-table-box ${entity.side || ""}"><h4>${escapeHtml(entity.name || "Unnamed entity")}</h4>${(entity.attributes || []).filter(a=>a.name).map(a=>`<div><span>${keyDisplay(a)}</span><small>${a.key || ""}</small></div>`).join("")}</div>${index === 0 && ordered.length > 1 ? `<div class="erd-link"><span>1</span><i></i><strong>${escapeHtml(state.oneToManyVerb || "relates to")}</strong><i></i><span>∞</span></div>` : ""}`).join("")}</div>`;
  };
  const renderBubble = () => {
    const entities = state.entities.slice(0,2);
    const width = 900, height = 520;
    const centers = entities.map((_,i) => ({x:i===0?245:655,y:260}));
    let svg = `<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Bubble style entity relationship diagram">`;
    if (entities.length === 2) svg += `<line x1="335" y1="260" x2="565" y2="260" class="erd-svg-link"/><text x="355" y="244" class="erd-cardinality">${entities[0].side==='one'?'1':entities[0].side==='many'?'M':'?'}</text><text x="535" y="244" class="erd-cardinality">${entities[1].side==='many'?'M':entities[1].side==='one'?'1':'?'}</text><text x="450" y="238" text-anchor="middle" class="erd-verb">${escapeHtml(state.oneToManyVerb || "relates to")}</text>`;
    entities.forEach((entity,i) => {
      const c=centers[i]; const attrs=(entity.attributes||[]).filter(a=>a.name); const radius=150;
      svg += `<rect x="${c.x-72}" y="${c.y-31}" width="144" height="62" rx="6" class="erd-entity-rect ${entity.side || ""}"/><text x="${c.x}" y="${c.y+7}" text-anchor="middle" class="erd-entity-text">${escapeHtml(entity.name || "Entity")}</text>`;
      attrs.forEach((attribute,j) => { const angle=(Math.PI*2*j/Math.max(attrs.length,1)) - Math.PI/2; const ax=c.x+Math.cos(angle)*radius; const ay=c.y+Math.sin(angle)*radius; const tx=c.x+Math.cos(angle)*78; const ty=c.y+Math.sin(angle)*78; svg += `<line x1="${tx}" y1="${ty}" x2="${ax}" y2="${ay}" class="erd-attribute-line"/><ellipse cx="${ax}" cy="${ay}" rx="63" ry="27" class="erd-attribute-bubble"/><text x="${ax}" y="${ay+5}" text-anchor="middle" class="erd-attribute-text ${attribute.key==='PK'?'pk':''}">${escapeHtml(attribute.name)}${attribute.key==='FK'?'*':''}</text>${attribute.key==='PK'?`<line x1="${ax-40}" y1="${ay+10}" x2="${ax+40}" y2="${ay+10}" class="erd-pk-underline"/>`:''}`; });
    });
    svg += `</svg><p class="erd-symbol-note"><strong>Bubble notation:</strong> primary keys are underlined; foreign keys are starred (*).</p>`;
