...
console.info(`Nat 5 Computing v${APP_VERSION}: Mastery Edition + Computing Arcade`);
...
async function routeSignedInUser(user) {
  try {
    ...
    const usesPasswordSignIn = providerIds.has("password") && !providerIds.has("google.com");
    if (isSchoolPupil && usesPasswordSignIn && !user.emailVerified) {
      ...
      showMessage("Please verify your email address before starting. We have sent another verification email; open the link in that email, then sign in again.", "info");
      await signOut(auth);
      return;
    }
...
async function createSchoolPupilProfile(user, joinData, code, displayName) {
  ...
  batch.set(doc(db, "users", user.uid), {
    ...
    joinCode: code,
    authMethod: (user.providerData || []).some(provider => provider.providerId === "google.com") ? "google" : "email-password",
    createdAt: serverTimestamp()
  });
...
elements.pupilGoogleLoginButton.addEventListener("click", async () => {
  clearMessage();
  elements.pupilGoogleLoginButton.disabled = true;
  ...
  try {
    ...
    if (!profileSnapshot.exists()) {
      await signOut(auth);
      showMessage("That Google account has not joined Nat 5 Computing yet. Choose Create pupil account, enter your class code and join with Google first.", "info");
      return;
    }
    if (profileSnapshot.data().role !== "student") {
      await signOut(auth);
      showMessage("That Google account is registered as a teacher. Use the Teacher sign-in button instead.", "error");
      return;
    }
    ...
    console.error(error);
    showMessage(friendlyAuthMessage(error, "Google pupil sign-in failed."), "error");
  } finally {
    accountSetupInProgress = false;
    elements.pupilGoogleLoginButton.disabled = false;
  }
});
...
elements.pupilLoginForm.addEventListener("submit", async event => {
  ...
  try {
    ...
    console.error(error);
    showMessage(friendlyAuthMessage(error, "The pupil account could not be opened."), "error");
  }
});
...
elements.pupilForgotPasswordButton.addEventListener("click", async () => {
  ...
  const email = normaliseEmail(elements.loginEmail.value);
  if (!email) {
    showMessage("Enter your email address first, then choose Forgot password?", "info");
...
elements.pupilGoogleRegisterButton.addEventListener("click", async () => {
  clearMessage();
  const code = normaliseClassCode(elements.registerClassCode.value);
  const displayName = elements.registerDisplayName.value.trim();
  if (code.length < 5 || !displayName) {
    showMessage("Enter your class code and display name before continuing with Google.", "error");
    return;
  }

  elements.pupilGoogleRegisterButton.disabled = true;
  accountSetupInProgress = true;
  try {
    ...
    if (existing.exists()) {
      if (existing.data().role !== "student") {
        await signOut(auth);
        showMessage("That Google account is already registered as a teacher.", "error");
        return;
      }
      accountSetupInProgress = false;
      showMessage("That Google pupil account already exists. Signing you in instead.", "success");
      await routeSignedInUser(user);
      return;
    }

    await createSchoolPupilProfile(user, joinData, code, displayName);
    accountSetupInProgress = false;
    showMessage("Google pupil account created successfully.", "success");
    await routeSignedInUser(user);
  } catch (error) {
    console.error(error);
    try { if (auth.currentUser) await signOut(auth); } catch (_) { /* ignore cleanup */ }
    showMessage(friendlyAuthMessage(error, "The Google pupil account could not be created."), "error");
  } finally {
    accountSetupInProgress = false;
    elements.pupilGoogleRegisterButton.disabled = false;
  }
});
...
elements.pupilRegisterForm.addEventListener("submit", async event => {
  ...
  if (!email) {
    showMessage("Enter the email address you want to use for sign-in and password recovery.", "error");
    return;
  }
  ...
  try {
    ...
    showMessage(
      verificationSent
        ? "Account created. Check your email and click the verification link, then come back here and sign in."
